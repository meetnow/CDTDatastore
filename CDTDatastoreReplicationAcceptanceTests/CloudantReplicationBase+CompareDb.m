...
#import <CDTDatastore/CDTDatastore+Internal.h>
        ...
        enumerateObjectsWithOptions:NSEnumerationConcurrent
                         usingBlock:^(id ob, NSUInteger idx, BOOL *stop) {
                           ...
                           NSArray *allRevisions =
                               [local activeRevisionsForDocumentId:document.docId];
                           ...
                           for (CDTDocumentRevision *currentRevision in allRevisions) {
                               ...
                               NSArray *localRevIds = [localRevIdsAcc
                               ...
                               NSDictionary *headers = @{ @"accept" : @"application/json" };
                               NSURL *docUrl =
                                   [databaseUrl URLByAppendingPathComponent:currentRevision.docId];
                               NSDictionary *json = [[UNIRest get:^(UNISimpleRequest *request) {
                                                      [request setUrl:[docUrl absoluteString]];
                                                      [request setHeaders:headers];
                                                      [request setParameters:@{
                                                          @"revs_info" : @"true",
                                                          @"rev" : currentRevision.revId
                                                      }];
                                                    }] asJson]
                                                        .body.object;
                               ...
                               NSArray *remoteRevIds = [remoteRevIdsAccumulator
                                   sortedArrayUsingSelector:@selector(localizedStandardCompare:)];

                               // CouchDB trims revision histories, so we should do that with the
                               // local array
                               // CouchDB will obviously return the most recent revIds, which will
                               // be at the
    ...
    [allLocalDocs
        enumerateObjectsWithOptions:NSEnumerationConcurrent
                         usingBlock:^(id ob, NSUInteger idx, BOOL *stop) {
                           ...
                           for (CDTAttachment *att in
                                [local attachmentsForRev:document error:nil]) {
                           ...
                           XCTAssertEqual(localAttachments.count, remoteAttachments.count,
                           ...
                           for (NSString *name in [localAttachments allKeys]) {
                               ...
                               NSString *localValue =
                                   [[NSString alloc] initWithData:localData
                               ...
                               NSData *remoteData =
                                   [NSData dataFromBase64String:rAttachment[@"data"]];
                               NSString *remoteValue =
                                   [[NSString alloc] initWithData:remoteData
                                                         encoding:NSUTF8StringEncoding];
                               XCTAssertEqualObjects(localValue, remoteValue,
                                                     @"Attachment data didn't match");
                               ...
                               //             NSString *remoteMD5Base64 = [digest
                               //             substringWithRange:r];
                               //             NSData *remoteMD5 = [NSData
                               //             dataFromBase64String:remoteMD5Base64];
                               //             STAssertEqualObjects(localMD5, remoteMD5, @"Attachment
                               //             MD5 didn't match");
                           }
                         }];
