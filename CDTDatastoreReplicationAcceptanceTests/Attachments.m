...
- (BOOL)isNumberOfAttachmentsForRevision:(CDTDocumentRevision *)rev equalTo:(NSUInteger)expected
{
...
- (void)pullFrom:(NSURL *)remoteDbURL to:(CDTDatastore *)local
{
    CDTReplicator *replicator =
        [self.replicatorFactory onewaySourceURI:remoteDbURL targetDatastore:local];
...
{
    CDTReplicator *replicator =
        [self.replicatorFactory onewaySourceDatastore:local targetURI:remoteDbURL];
    [replicator startWithError:nil];
    while (replicator.isActive) {
        [NSThread sleepForTimeInterval:1.0f];
    }
...
- (void)testSavedHttpAttachmentWithRemote
{
    ...
    NSURL *docURL = [remoteDbURL URLByAppendingPathComponent:docId];
    NSURLComponents *docURLComponents =
        [NSURLComponents componentsWithURL:docURL resolvingAgainstBaseURL:NO];
    ...
    CDTDocumentRevision *remoteRevision =
        [CDTDocumentRevision createRevisionFromJson:responseDict forDocument:docURL error:&error];
    ...
    XCTAssertEqualObjects([@"blahblah" dataUsingEncoding:NSUTF8StringEncoding],
                          [remoteAttachment dataFromAttachmentContent],
                          @"Attachment content was not equal");
...
- (void)testReplicateSeveralRemoteDocumentsWithAttachments
{
    ...
    rev = [self.datastore getDocumentWithId:@"attachments1" error:nil];
    XCTAssertTrue([self isNumberOfAttachmentsForRevision:rev equalTo:(NSUInteger)1],
                  @"Incorrect number of attachments");

    rev = [self.datastore getDocumentWithId:@"attachments3" error:nil];
    XCTAssertTrue([self isNumberOfAttachmentsForRevision:rev equalTo:(NSUInteger)3],
                  @"Incorrect number of attachments");

    rev = [self.datastore getDocumentWithId:@"attachments4" error:nil];
...
- (void)testReplicateManyRemoteAttachments
{
    ...
    CDTDocumentRevision *rev;
    rev = [self.datastore getDocumentWithId:docId error:nil];
    XCTAssertTrue([self isNumberOfAttachmentsForRevision:rev equalTo:nAttachments],
                  @"Incorrect number of attachments");
...
- (void)testReplicateRemoteDocumentUpdate
{
    ...
    CDTDocumentRevision *rev = [self.datastore getDocumentWithId:docId error:nil];

    NSString *updatedContent = @"updatedContent";
    NSData *data = [updatedContent dataUsingEncoding:NSUTF8StringEncoding];
    CDTAttachment *attachment =
        [[CDTUnsavedDataAttachment alloc] initWithData:data name:attachmentName type:@"text/plain"];
    NSError *error = nil;
...
- (void)testRevposIssueFixed
{
    ...
    revId = [self createRemoteDocumentWithId:docId body:dict databaseURL:remoteDbURL];
    ...
    [self copyRemoteDocumentWithId:docId toId:copiedDocId databaseURL:remoteDbURL];
    ...
    XCTAssertEqualObjects(json[@"_attachments"][@"txtDoc"][@"revpos"], @(2),
                          @"revpos not expected");
    ...
    rev = [self.datastore getDocumentWithId:copiedDocId error:nil];
    XCTAssertTrue([self isNumberOfAttachmentsForRevision:rev equalTo:(NSUInteger)1],
                  @"Incorrect number of attachments");
