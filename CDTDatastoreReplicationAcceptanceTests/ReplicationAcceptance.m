 ...
 */
- (CDTReplicator *)pullFromRemote { return [self pullFromRemoteWithFilter:nil params:nil]; }
 ...
 */
- (CDTReplicator *)pushToRemote { return [self pushToRemoteWithFilter:nil params:nil]; }
/**
 Create a new replicator, and wait for replication from the local database to complete.
 */
- (CDTReplicator *)pushToRemoteWithFilter:(CDTFilterBlock)filter params:(NSDictionary *)params
{
    CDTPushReplication *push =
        [CDTPushReplication replicationWithSource:self.datastore
...
- (void)testPushLotsOfOneRevDocuments
{
    ...
    BOOL same = [self compareDatastore:self.datastore withDatabase:self.primaryRemoteDatabaseURL];
    XCTAssertTrue(same, @"Remote and local databases differ");

    XCTAssertEqual(n_docs, (NSUInteger)replicator.changesTotal,
                   @"total number of changes mismatch");
    XCTAssertEqual(n_docs, (NSUInteger)replicator.changesProcessed,
                   @"processed number of changes mismatch");
}
...
- (void)testPullErrorsWhenLocalDatabaseIsDeleted
{
    ...
    XCTAssertTrue(n_docs != (NSUInteger)replicator.changesProcessed,
                  @"changesProcessed: %ld, n_docs %ld", replicator.changesProcessed, n_docs);
...
- (void)testSyncReplicationErrorsWhenLocalDatabaseDeleted_pushDelegateDeletes
{
    ...
    XCTAssertTrue(n_docs != (NSUInteger)pullReplicator.changesTotal,
                  @"changesTotal: %ld, n_docs %ld", pullReplicator.changesTotal, n_docs);

    XCTAssertTrue(n_docs != (NSUInteger)pullReplicator.changesProcessed,
                  @"changesProcessed: %ld, n_docs %ld", pullReplicator.changesProcessed, n_docs);
    ...
    XCTAssertTrue(n_docs != (NSUInteger)pushReplicator.changesTotal,
                  @"changesTotal: %ld, n_docs %ld", pushReplicator.changesTotal, n_docs);

    XCTAssertTrue(n_docs != (NSUInteger)pushReplicator.changesProcessed,
                  @"changesProcessed: %ld, n_docs %ld", pushReplicator.changesProcessed, n_docs);
...
- (void)testPullFilteredReplication
{
    ...
    NSString *ddoc =
        [NSString stringWithFormat:@"ddoc-%@", [CloudantReplicationBase generateRandomString:5]];
    ...
    // check for each doc
    NSArray *docids = @[
        [NSString stringWithFormat:@"doc-%i", 1],
    ...
    XCTAssertTrue(localDocs.count == totalReplicated, @"unexpected number of docs: %@",
                  localDocs.count);
...
- (void)testPushFilteredReplicationAllowOne
{
    ...
    [self assertRemoteDatabaseHasDocCount:1 deletedDocs:0];
...
- (void)testPushFilteredReplicationAllowSome
{
    ...
    [self assertRemoteDatabaseHasDocCount:7 deletedDocs:0];
}
...
- (void)testPushStateAfterStopping
{
    ...
    XCTAssertEqual(replicator.state, CDTReplicatorStateStopped,
                   @"expected a different state: %d (%@)", replicator.state,
                   [CDTReplicator stringForReplicatorState:replicator.state]);
...
- (void)testPushLargeRevTree
{
    ...
    XCTAssertEqual(self.datastore.documentCount, (NSUInteger)1,
    ...
    XCTAssertEqual([jsonResponse[@"_revisions"][@"ids"] count], (NSUInteger)1000,
                   @"Wrong number of revs");
...
- (void)testPullModifySeveralRevsPush
{
    ...
    [self assertRemoteDatabaseHasDocCount:n_docs deletedDocs:0];
    ...
    for (int i = 1; i < n_docs + 1; i++) {
        ...
        XCTAssertEqual([jsonResponse[@"_revisions"][@"ids"] count], (NSUInteger)n_mods,
                       @"Wrong number of revs");
        XCTAssertTrue([jsonResponse[@"_rev"] hasPrefix:@"10"],
                      @"Not all revs seem to be replicated");
    }
...
- (void)testPullDeleteAllPush
{
    ...
    [self assertRemoteDatabaseHasDocCount:0 deletedDocs:n_docs];
...
- (void)test_pushDocsAsWritingThem
{
    ...
    [self performSelectorInBackground:@selector(
                                          pushDocsAsWritingThem_populateLocalDatabaseThenSignal:)
                           withObject:monitor];
...
- (void)test_pullDocsWhileWritingOthers
{
    ...
    [self
        performSelectorInBackground:@selector(
                                        pullDocsWhileWritingOthers_populateLocalDatabaseThenSignal:)
                         withObject:monitor];

    [monitor wait];

    XCTAssertEqual(self.datastore.documentCount, (NSUInteger)n_docs * 2,
                   @"Wrong number of local docs");
...
- (void)test_pullDocsWhileWritingOthersWriteToThirdDB
{
    ...
    NSString *thirdDatabaseName =
        [NSString stringWithFormat:@"%@-test-third-database-%@", self.remoteDbPrefix,
                                   [CloudantReplicationBase generateRandomString:5]];
    ...
    CDTPushReplication *push =
        [CDTPushReplication replicationWithSource:self.datastore target:thirdDatabase];
    ...
    BOOL same = [self compareDatastore:self.datastore withDatabase:thirdDatabase];
    XCTAssertTrue(same, @"Remote and local databases differ");
...
- (void)test_pullDocsWhileWritingSame
{
    ...
    // Create documents that don't conflict as we pull
    [self performSelectorInBackground:@selector(
                                          pullDocsWhileWritingSame_populateLocalDatabaseThenSignal:)
                           withObject:monitor];
...
- (void)testCreateReplicationWithExtraHeaders
{
    ...
    NSString *userAgent =
        [NSString stringWithFormat:@"%@/testCreateReplicationWithExtraHeaders",
...
- (void)testMultiThreadedReplication
{
    ...
    CDTDatastore *secondDatastore =
        [self.factory datastoreNamed:@"test2" withEncryptionKeyProvider:self.provider error:nil];
    CDTPullReplication *secondPull =
        [CDTPullReplication replicationWithSource:self.primaryRemoteDatabaseURL
    ...
    while (firstReplicator.isActive || secondReplicator.isActive) {
        [NSThread sleepForTimeInterval:1.0f];
        NSLog(@" 1st replicator -> %@",
              [CDTReplicator stringForReplicatorState:firstReplicator.state]);
        NSLog(@"    changes Processed: %ld", firstReplicator.changesProcessed);
        NSLog(@" 2nd replicator -> %@",
              [CDTReplicator stringForReplicatorState:secondReplicator.state]);
...
{
    // number of remote docs is not a mutliple of 200, which is the default batch size of docs
    // fetched
    ...
    TDReplicatorManager *tdrepman =
        [[TDReplicatorManager alloc] initWithDatabaseManager:self.factory.manager];
    ...
    NSURL *docURL =
        [self.primaryRemoteDatabaseURL URLByAppendingPathComponent:remoteCheckpointPath];
    NSDictionary *headers =
        @{ @"accept" : @"application/json",
           @"content-type" : @"application/json" };
    UNIHTTPJsonResponse *response = [[UNIRest get:^(UNISimpleRequest *request) {
...
{
    // number of remote docs is not a multiple of 200, which is the default batch size of docs
    // fetched
    // during replication. This gives a more realistic simulation. The changes feed tracker
    // will fire off one more request to _changes when the returned number of changes does
    // not equal it's limit.
...
- (void)testURLConnectionChangeTrackerWithRealRemoteUsingAuthorizer
{
    ...
    NSURL *url =
        [NSURL URLWithString:@"https://mikerhodescloudant.cloudant.com/shared_todo_sample"];
    CDTURLSession *session = [[CDTURLSession alloc] init];
    TDChangeTracker *changeTracker = [[TDChangeTracker alloc] initWithDatabaseURL:url
                                                                             mode:kOneShot
                                                                        conflicts:YES
    ...
    NSURLCredential *cred =
        [NSURLCredential credentialWithUser:[[self sharedDemoURL] user]
...
- (void)testURLConnectionChangeTrackerWithBadCredentials
{
    ...
    XCTAssertNotNil(changeTracker.authorizer,
                    @"Authorizer object should be not nil. "
