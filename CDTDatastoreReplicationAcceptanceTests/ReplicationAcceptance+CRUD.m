...
- (void)createLocalDocs:(NSInteger)count { [self createLocalDocs:count suffixFrom:0]; }
...
- (void)createLocalDocWithId:(NSString *)docId revs:(NSInteger)n_revs
{
    ...
    XCTAssertTrue([rev.revId hasPrefix:revPrefix], @"Unexpected current rev in local document, %@",
                  rev.revId);
}

- (CDTDocumentRevision *)addRevsToDocumentRevision:(CDTDocumentRevision *)rev
                                             count:(NSInteger)n_revs
{
    NSError *error;
...
- (void)createRemoteDocs:(NSInteger)count { [self createRemoteDocs:count suffixFrom:0]; }
- (void)createRemoteDocs:(NSInteger)count suffixFrom:(NSInteger)start
{
    ...
    UNIHTTPJsonResponse *response = [[UNIRest postEntity:^(UNIBodyRequest *request) {
      ...
      [request setBody:[NSJSONSerialization dataWithJSONObject:bulk_json options:0 error:nil]];
    }] asJson];
...
- (void)createRemoteDocWithId:(NSString *)docId revs:(NSInteger)n_revs
{
    ...
    NSString *revPrefix = [NSString stringWithFormat:@"%li", (long)n_revs];
    XCTAssertTrue([revId hasPrefix:revPrefix], @"Unexpected current rev in local document, %@",
                  revId);
}
...
- (void)assertRemoteDatabaseHasDocCount:(NSInteger)count deletedDocs:(NSInteger)deleted
{
    ...
    XCTAssertEqual(count, [dbMeta[@"doc_count"] integerValue], @"Wrong number of remote docs");
    XCTAssertEqual(deleted, [dbMeta[@"doc_del_count"] integerValue],
