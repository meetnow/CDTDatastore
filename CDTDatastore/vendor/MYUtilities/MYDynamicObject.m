...
- (BOOL)setValue:(id)value ofProperty:(NSString*)property { return NO; }
...
NS_INLINE BOOL isGetter(const char* name)
{
    ...
    if (strncmp(name, "get", 3) == 0) return NO;  // Ignore "getXXX" variants of getter syntax
    return YES;
}
...
+ (NSSet*)propertyNames
{
    ...
    if (!classToNames) classToNames = [[NSMutableDictionary alloc] init];

    if (self == [MYDynamicObject class]) return [NSSet set];
    ...
    if (propertiesExcludingSuperclass) {
        ...
        while (*propertyPtr)
            [propertyNames
                addObject:[NSString stringWithUTF8String:property_getName(*propertyPtr++)]];
    ...
    while ((attribute = strsep(&state, ",")) != NULL) {
        switch (attribute[0]) {
            case 'T':  // Property type in @encode format
                result = (const char*)[
                    [NSData dataWithBytes:(attribute + 1) length:strlen(attribute)] bytes];
                break;
...
static BOOL getPropertyInfo(Class cls, NSString* propertyName, BOOL setter, Class* declaredInClass,
...
+ (Class)classOfProperty:(NSString*)propertyName
{
    ...
    const char* propertyType;
    if (!getPropertyInfo(self, propertyName, NO, &declaredInClass, &propertyType)) return Nil;
...
+ (BOOL)resolveInstanceMethod:(SEL)sel
{
    ...
    if (propertyType) {
        Warn(@"Dynamic property %@.%@ has type '%s' unsupported by %@", self, key, propertyType,
             self);
    }
    return NO;
}
