    ...
    if (basicClauses.count > 0) {
        if (query[AND]) {
            ...
            if (!chosenIndex) {
                ...
                CDTQSqlParts *select =
                    [CDTQQuerySqlTranslator selectStatementForAndClause:basicClauses
            ...
            for (NSDictionary *clause in basicClauses) {
                ...
                NSString *chosenIndex =
                    [CDTQQuerySqlTranslator chooseIndexForAndClause:wrappedClause
                                                        fromIndexes:indexes];
                if (!chosenIndex) {
                    state.atLeastOneIndexMissing = YES;
    ...
    [clauses enumerateObjectsUsingBlock:^void(id obj, NSUInteger idx, BOOL *stop) {
      ...
      if ([field isEqualToString:OR]) {
          CDTQQueryNode *orNode =
              [CDTQQuerySqlTranslator translateQuery:clauses[idx] toUseIndexes:indexes state:state];
        ...
        if ([operator isEqualToString:NOT]) {
            ...
            if([operator isEqualToString:EXISTS]){
                ...
                if ([operator isEqualToString:IN]) {
                    ...
                    placeholder = [CDTQQuerySqlTranslator placeholdersForList:inList
            ...
            if ([operator isEqualToString:EXISTS]){
                ...
                sqlClause = [NSString
                    stringWithFormat:@"\"%@\" %@ %@", fieldName, sqlOperator, placeholder];
                [sqlClauses addObject:sqlClause];
            }
        }
    }
...
{
    NSString *whereForSubSelect =
        [NSString stringWithFormat:@"\"%@\" %@ %@", fieldName, sqlOperator, operand];
    NSString *subSelect =
        [NSString stringWithFormat:@"SELECT _id FROM %@ WHERE %@", tableName, whereForSubSelect];
...
+ (CDTQSqlParts *)selectStatementForAndClause:(NSArray *)clause usingIndex:(NSString *)indexName
{
    ...
    CDTQSqlParts *where =
        [CDTQQuerySqlTranslator wherePartsForAndClause:clause usingIndex:indexName];
