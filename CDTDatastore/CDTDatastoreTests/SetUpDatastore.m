 ...
 */
- (void)testSetupAndTeardownDatastoreFactory { XCTAssertNotNil(self.factory, @"Factory is nil"); }
/**
...
- (void)testSetupAndDeleteDatastore
{
    ...
    XCTAssertTrue([fm fileExistsAtPath:[dir stringByAppendingPathComponent:dbNameFull]],
                  @"db file does not exist");
    XCTAssertTrue([fm fileExistsAtPath:[dir stringByAppendingPathComponent:dbNameExtensions]],
                  @"extensions dir does not exist");
    XCTAssertTrue([fm fileExistsAtPath:[dir stringByAppendingPathComponent:dbNameAttachments]],
                  @"attachments dir does not exist");

    // delete datastore
    XCTAssertTrue([self.factory deleteDatastoreNamed:dbName error:&error],
                  @"deleteDatastoreNamed did not return true");

    // check various files/dirs have been deleted
    XCTAssertFalse([fm fileExistsAtPath:[dir stringByAppendingPathComponent:dbNameFull]],
                   @"db file was not deleted");
    XCTAssertFalse([fm fileExistsAtPath:[dir stringByAppendingPathComponent:dbNameExtensions]],
                   @"extensions dir was not deleted");
    XCTAssertFalse([fm fileExistsAtPath:[dir stringByAppendingPathComponent:dbNameAttachments]],
                   @"attachments dir was not deleted");
}
...
- (void)testDeleteDatastoreReturnsErrorIfNameIsNotValid
{
    ...
    BOOL isExpectedError =
        (error && ([error.domain isEqualToString:CDTDatastoreErrorDomain]) && (error.code == 404));
