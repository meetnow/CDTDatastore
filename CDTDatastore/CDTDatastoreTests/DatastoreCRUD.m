...
- (void)testDocumentWithInfinityValue
{
    ...
    CDTDocumentRevision *rev = [self.datastore createDocumentFromRevision:revision error:&error];

    XCTAssertNil(rev, @"revision is not nil");
    XCTAssertNotNil(error, @"Error should be set");
    XCTAssertEqual(400, error.code, @"Error code should be 400, but was %ld", (long)error.code);
}
...
- (void)testDocumentRevisionFactoryValidData
{
    ...
    CDTDocumentRevision *rev = [CDTDocumentRevision
        createRevisionFromJson:dict
                   forDocument:[NSURL URLWithString:@"http://localhost:5984/temp/doc"]
                         error:&error];

    XCTAssertNil(error, @"Error occured creating document with valid data");
    XCTAssertNotNil(rev, @"Revision was nil");
    XCTAssertEqualObjects(@"someIdHere", rev.docId,
                          @"docId was different, expected someIdHere actual %@", rev.docId);
    XCTAssertEqualObjects(
        @"3-750dac460a6cc41e6999f8943b8e603e", rev.revId,
        @"Revision was different expected 3-750dac460a6cc41e6999f8943b8e603e actual %@", rev.revId);
...
- (void)testCreateOneDocumentSQLEntries
{
    ...
    [self.datastore
            documentCount];  // this calls ensureDatabaseOpen, which calls TD_Database open:, which
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      NSArray *expectedRows = @[ @[ @"docId" ], @[ testDocId ] ];
      NSError *validationError;
      XCTAssertTrue([dc checkDatabase:db table:@"docs" hasRows:expectedRows error:&validationError],
                    @"%@", [dc formattedErrors:validationError]);
    }];

    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      NSString *revId = @"1-1f7588ca02054efe626a6e440a431861";
      NSData *json = [TDJSON dataWithJSONObject:@{ key : value } options:0 error:nil];
      NSArray *expectedRows = @[
          @[ @"doc_id", @"sequence", @"revid", @"current", @"deleted", @"json" ],
          @[ @(1), @1, revId, @YES, @NO, json ]
      ];
      NSError *validationError;
      XCTAssertTrue([dc checkDatabase:db table:@"revs" hasRows:expectedRows error:&validationError],
                    @"%@", [dc formattedErrors:validationError]);
    }];
}
...
- (void)testCannotCreateNewDocWithoutUniqueID
{
    ...
    XCTAssertNil(
        ob, @"CDTDocumentRevision object was not nil when creating second doc with same doc_id");
}
...
- (void)testCreateDocumentWithId
{
    ...
    XCTAssertEqualObjects(ob.docId, retrieved.docId,
                          @"Object retrieved from database has wrong docid");
    XCTAssertEqualObjects(ob.revId, retrieved.revId,
                          @"Object retrieved from database has wrong revid");
...
- (void)testGetDocumentWithIdAndRev
{
    ...
    CDTDocumentRevision *retrieved =
        [self.datastore getDocumentWithId:docId rev:revId error:&error];
    XCTAssertNil(error, @"Error retrieving document");
...
- (void)testGetDocumentsWithIds
{
    ...
    for (NSNumber *index in @[ @5, @7, @12, @170 ]) {
        ...
        XCTAssertNotNil(retrieved, @"retrieved object was nil");
        XCTAssertEqualObjects(retrieved.docId, docId,
                              @"Object retrieved from database has wrong docid");
        const NSUInteger expected_count = 2;
        XCTAssertEqual(retrieved.body.count, expected_count, @"Object from database has != 2 keys");
        XCTAssertEqualObjects(retrieved.body[@"hello"], @"world",
                              @"Object from database has wrong data");
        XCTAssertEqualObjects(retrieved.body[@"index"], index,
                              @"Object from database has wrong data");
...
- (void)testGetCompactedDocumentRev
{
    ...
    NSArray *ids = @[
        @"compactrevtest_doc_1",
        @"compactrevtest_doc_2",
        @"compactrevtest_doc_3",
        @"compactrevtest_doc_4"
    ];
    NSError *error;
    ...
    for (id anid in ids) {
        lastDocForId[anid] = [NSMutableDictionary dictionaryWithDictionary:@{
            @"numUpdates" : [NSNumber numberWithUnsignedInt:0],
            @"json" : @{},
            @"lastrev" : @""
        }];
    }
    ...
    for (int i = 0; i < totalUpdates; i++) {
        ...
        if ([lastDocForId[randomId][@"numUpdates"] intValue] == 0) {
            ...
            cdt_rev = [self.datastore getDocumentWithId:randomId error:&error];
        ...
        XCTAssertTrue([lastDocForId[randomId][@"numUpdates"] unsignedIntegerValue] ==
                          [TD_Revision generationFromRevID:cdt_rev.revId],
                      @"rev prefix value does not equal expected number of updates");
    }

    NSDictionary *modifiedCounts =
        @{ @"docs" : @4,
           @"revs" : [NSNumber numberWithInt:totalUpdates] };
    ...
    XCTAssertTrue([TDStatusToNSError(statusResults, nil) code] == 200, @"TDStatusAsNSError: %@",
    ...
    for (NSString *aDocId in ids) {
        ...
        if (numUpdates > 0) {
            ...
            XCTAssertEqualObjects(lastRev, cdt_rev.revId,
                                  @"Object retrieved from database has wrong revid");
            XCTAssertEqualObjects(cdt_rev.body, lastRecordedJsonDoc,
                                  @"Object from database has wrong data");
            XCTAssertTrue(numUpdates == [TD_Revision generationFromRevID:cdt_rev.revId],
                          @"rev prefix value does not equal expected number of updates");
        }
    }
    ...
    for (id aDocId in ids) {
        [self.dbutil.queue inDatabase:^(FMDatabase *db) {
          ...
          FMResultSet *result =
              [db executeQuery:@"select sequence, revs.doc_id as docnum, docs.docid as docid, "
                               @"revid, json, parent, deleted from revs, docs where revs.doc_id = "
                               @"docs.doc_id and docs.docid = (?) order by sequence",
                               aDocId];
          ...
          while ([result next]) {
              ...
              XCTAssertTrue([TD_Revision generationFromRevID:revid] - 1 == revPrefix,
                            @"revision out of order: Found Rev: %@ and previous prefix %@", revid,
                            revPrefix);
              revPrefix = [TD_Revision generationFromRevID:revid];

              XCTAssertFalse([result boolForColumn:@"deleted"], @"deleted? %@",
                             [result stringForColumn:@"deleted"]);

              // this should be the first row in the result
              if (docNum == -1) {
                  docNum = [result intForColumn:@"docnum"];  // make sure docNum is always the same
                  XCTAssertEqualObjects([result objectForColumnName:@"parent"], [NSNull null],
                                        @"parent: %@", [result objectForColumnName:@"parent"]);
              } else {
                  XCTAssertTrue([result intForColumn:@"docnum"] == docNum, @"%@",
                                [result stringForColumn:@"docnum"]);
                  XCTAssertTrue([result intForColumn:@"docnum"] <= ids.count, @"%@",
                                [result stringForColumn:@"docnum"]);
              }
              ...
              if ([revid isEqualToString:lastDocForId[aDocId][@"lastrev"]]) {
                  ...
                  NSDictionary *jsonDoc = [TDJSON JSONObjectWithData:[result dataForColumn:@"json"]
                                                             options:TDJSONReadingMutableContainers
                                                               error:&error];
                  XCTAssertTrue([jsonDoc isEqualToDictionary:lastDocForId[aDocId][@"json"]],
                                @"Found: %@", jsonDoc);
                  if (jsonDoc) foundJSON = YES;
              } else {
                  // This was slightly unexpected. When TD_Database deletes, the JSON becomes an
                  // empty NSData
                  // object. But when compacting, it sets it to null. See TD_Database+Insertion
                  // compact.
                  XCTAssertEqualObjects([result objectForColumnName:@"json"], [NSNull null],
                                        @"Expected revs.json to be empty NSData. Found %@",
...
- (void)test_getAllDocumentsOffsetLimitDescending
{
    ...
    //[self getAllDocuments_testCountAndOffset:objectCount expectedDbObjects:reversedObjects
    // descending:YES];
}
...
- (void)testGetAllDocumentIds
{
    ...
    XCTAssertEqual([self.datastore getAllDocumentIds].count, objectCount,
    ...
    for (int i = 0; i < objectCount; i++) {
        ...
        XCTAssertTrue([found isEqualToString:expected], @"Expecting %@ but found %@", expected,
                      found);
    }
}

- (void)assertIdAndRevisionAndShallowContentExpected:(CDTDocumentRevision *)expected
                                              actual:(CDTDocumentRevision *)actual
{
    ...
    for (NSString *key in [expectedDict keyEnumerator]) {
        ...
        XCTAssertEqualObjects([actualDict objectForKey:key], [expectedDict objectForKey:key],
                              @"Actual value didn't match expected value");
    }
}

- (void)getAllDocuments_testCountAndOffset:(int)objectCount
                         expectedDbObjects:(NSArray *)expectedDbObjects
                                descending:(Boolean)descending
{
    ...
    // Count
    count = 10;
    result = [self.datastore getAllDocumentsOffset:offset limit:count descending:descending];
    ...
    offset = objectCount + 5;
    count = 10;
    result = [self.datastore getAllDocumentsOffset:offset limit:count descending:descending];
    [self getAllDocuments_compareResultExpected:expectedDbObjects actual:result count:0 offset:0];
}

- (void)getAllDocuments_compareResultExpected:(NSArray *)expectedDbObjects
                                       actual:(NSArray *)result
                                        count:(int)count
                                       offset:(int)offset
{
...
- (void)testUpdatingSingleDocument
{
    ...
    XCTAssertEqualObjects(ob.docId, retrieved.docId,
                          @"Object retrieved from database has wrong docid");
    XCTAssertEqualObjects(ob2.revId, retrieved.revId,
                          @"Object retrieved from database has wrong revid");
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      NSError *validationError;
      NSArray *expectedRows = @[ @[ @"doc_id", @"docid" ], @[ expectedDoc_id, docId ] ];
      XCTAssertTrue([dc checkDatabase:db table:@"docs" hasRows:expectedRows error:&validationError],
                    @"%@", [dc formattedErrors:validationError]);
    }];
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      NSData *json1 = [TDJSON dataWithJSONObject:@{ key1 : value1 } options:0 error:nil];

      NSString *revId2 = @"2-be2c05cd85b8468ce92b04e001b0e923";
      NSData *json2 = [TDJSON dataWithJSONObject:@{ key2 : value2 } options:0 error:nil];
      NSArray *expectedRows = @[
          @[ @"doc_id", @"sequence", @"revid", @"current", @"deleted", @"json" ],
...
- (void)testUpdateWithNilDocumentBody
{
    ...
    XCTAssertTrue(error.code == 400, @"Error was not a 400. Found %ld", error.code);
    XCTAssertNil(ob2,
                 @"CDTDocumentRevision object was not nil when updating with nil document body");
...
- (void)testMultipleUpdates
{
    ...
    NSDictionary *modifiedCount =
        @{ @"docs" : @1,
           @"revs" : [[NSNumber alloc] initWithInt:numOfUpdates + 1] };
    [self.dbutil checkTableRowCount:initialRowCount modifiedBy:modifiedCount];
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      NSError *validationError;
      NSArray *expectedRows = @[ @[ @"docId" ], @[ ob.docId ] ];
      XCTAssertTrue([dc checkDatabase:db table:@"docs" hasRows:expectedRows error:&validationError],
                    @"%@", [dc formattedErrors:validationError]);
    }];
    ...
    [queue inDatabase:^(FMDatabase *db) {
      ...
      for (int counter = 1; counter <= numOfUpdates + 1; counter++) {
          ...
          NSRegularExpression *revIdRegEx =
              [NSRegularExpression regularExpressionWithPattern:revId options:0 error:nil];
          ...
          NSData *json = [TDJSON dataWithJSONObject:expectedDict options:0 error:nil];

          NSArray *row =
              @[ @(1), @(expectedSeq), expectedParent, revIdRegEx, @(expectedCurrent), @NO, json ];
...
- (void)testUpdatingWithUnderscoreDeleteFieldDoesNotDelete
{
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      NSError *validationError;
      NSArray *expectedRows = @[ @[ @"doc_id", @"docid" ], @[ @1, docId ] ];
      XCTAssertTrue([dc checkDatabase:db table:@"docs" hasRows:expectedRows error:&validationError],
                    @"%@", [dc formattedErrors:validationError]);
    }];
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      NSString *revId = @"1-1f7588ca02054efe626a6e440a431861";
      NSData *json = [TDJSON dataWithJSONObject:@{ key1 : value1 } options:0 error:nil];
      NSArray *expectedRows = @[
          @[ @"doc_id", @"sequence", @"revid", @"current", @"deleted", @"json" ],
          @[ @(1), @1, revId, @YES, @NO, json ],
      ];
...
- (void)testDeletedItem404
{
    ...
    CDTDocumentRevision *tmp = [self.datastore getDocumentWithId:rev.docId error:&error];
...
- (void)testDeletedFlagOnDocumentRevision
{
    ...
    CDTDocumentRevision *tmp =
        [self.datastore getDocumentWithId:rev.docId rev:rev.revId error:&error];
...
- (void)testDeleteDocument
{
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      [result next];
      XCTAssertEqualObjects(docId, [result stringForColumn:@"docid"], @"%@ != %@", docId,
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      XCTAssertEqual(doc_id_inDocsTable, [result intForColumn:@"doc_id"], @"%d != %d",
                     doc_id_inDocsTable, [result intForColumn:@"doc_id"]);
      XCTAssertTrue([result intForColumn:@"sequence"] == 1, @"%d",
                    [result intForColumn:@"sequence"]);
      XCTAssertTrue([TD_Revision generationFromRevID:[result stringForColumn:@"revid"]] == 1,
                    @"rev: %@", [result stringForColumn:@"revid"]);
      ...
      XCTAssertEqualObjects([result objectForColumnName:@"parent"], [NSNull null], @"Found %@",
                            [result objectForColumnName:@"parent"]);
      NSDictionary *jsonDoc = [TDJSON JSONObjectWithData:[result dataForColumn:@"json"]
                                                 options:TDJSONReadingMutableContainers
                                                   error:&error];
      XCTAssertTrue([jsonDoc isEqualToDictionary:@{
                        @"hello" : @"world"
                    }],
                    @"JSON %@. NSError %@", jsonDoc, error);

      // next row
      XCTAssertTrue([result next], @"Didn't find the second row in the revs table");
      XCTAssertEqual(doc_id_inDocsTable, [result intForColumn:@"doc_id"], @"%d != %d",
                     doc_id_inDocsTable, [result intForColumn:@"doc_id"]);
      XCTAssertTrue([result intForColumn:@"sequence"] == 2, @"%d",
                    [result intForColumn:@"sequence"]);
      XCTAssertTrue([TD_Revision generationFromRevID:[result stringForColumn:@"revid"]] == 2,
                    @"rev: %@", [result stringForColumn:@"revid"]);
      ...
      XCTAssertTrue([result intForColumn:@"parent"] == 1, @"Found %@",
      ...
      XCTAssertNil([result stringForColumn:@"doc_id"], @"after [result next], doc_id is %@",
                   [result stringForColumn:@"doc_id"]);
      XCTAssertNil([result stringForColumn:@"revid"], @"after [result next],  revid is %@",
                   [result stringForColumn:@"revid"]);
...
- (void)testDeleteUpdateDocument
{
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      jsonDoc = [TDJSON JSONObjectWithData:[result dataForColumn:@"json"]
                                   options:TDJSONReadingMutableContainers
                                     error:&error];
      XCTAssertTrue([jsonDoc isEqualToDictionary:@{key2 : value2}], @"JSON %@. NSError %@", jsonDoc,
                    error);
      ...
      XCTAssertEqual(doc_id_inDocsTable, [result intForColumn:@"doc_id"], @"%d != %d",
                     doc_id_inDocsTable, [result intForColumn:@"doc_id"]);
      XCTAssertTrue([result intForColumn:@"sequence"] == 3, @"%d",
                    [result intForColumn:@"sequence"]);
      XCTAssertTrue([TD_Revision generationFromRevID:[result stringForColumn:@"revid"]] == 3,
                    @"rev: %@", [result stringForColumn:@"revid"]);
      XCTAssertTrue([result boolForColumn:@"current"], @"%@", [result stringForColumn:@"current"]);
      XCTAssertTrue([result boolForColumn:@"deleted"], @"%@", [result stringForColumn:@"current"]);
      XCTAssertTrue([result intForColumn:@"parent"] == 2, @"Found %d",
                    [result intForColumn:@"parent"]);
...
- (void)testUpdateDeleteUpdateOldRev
{
    ...
    // inconsitent with error reported by cloudant/couchdb. cloudant/couch returns 409, TD* retunrs
    // 404
...
- (void)testGetAllDocsDoesntFindDeletedDocs
{
    ...
    for (int i = 0; i < objectCount; i++) {
        if (arc4random_uniform(100) < 30) {  // delete ~30% of the docs
            ...
            CDTDocumentRevision *deleted =
                [self.datastore deleteDocumentFromRevision:ob error:&error];
    ...
    int offset = 0;
    NSArray *result;
    result = [self.datastore getAllDocumentsOffset:offset limit:objectCount + 10 descending:NO];

    // compare the returned revs to the expected with getAllDocuments
    [self getAllDocuments_compareResultExpected:notDeletedDbObjects
                                         actual:result
                                          count:(int)notDeletedDbObjects.count
                                         offset:offset];
    ...
    for (CDTDocumentRevision *aRev in result) {
        for (NSDictionary *aDeletedDict in deletedDbDicts) {
            XCTAssertFalse(aDeletedDict == aRev.body, @"Found equal pointers. %d == %d", aRev.body,
                           aDeletedDict);
            XCTAssertFalse([aDeletedDict isEqualToDictionary:aRev.body],
                           @"Found deleted dictionary in results");
        }
        // is this the same as above?
        XCTAssertNil([deletedDbDicts member:aRev.body], @"Found result in deleted set: %@",
                     aRev.body);
    }
    ...
    for (CDTDocumentRevision *aRev in deletedDbObjects) {
        NSError *error;
        CDTDocumentRevision *retrieved =
            [self.datastore getDocumentWithId:aRev.docId rev:aRev.revId error:&error];
        ...
        BOOL found = NO;
        for (NSDictionary *aDeletedDict in deletedDbDicts) {
            if ([aDeletedDict isEqualToDictionary:retrieved.body]) found = YES;
        }
        ...
        [queue inDatabase:^(FMDatabase *db) {
          ...
          FMResultSet *result = [db
              executeQuery:
                  @"select * from revs, docs where revs.doc_id = docs.doc_id and docs.docid = (?)",
                  aRev.docId];
          int count = 0;
          while ([result next]) {
              count++;
              if (count == 2) {
                  ...
                  NSDictionary *jsonDoc = [TDJSON JSONObjectWithData:[result dataForColumn:@"json"]
                                                             options:TDJSONReadingMutableContainers
                                                               error:&error];
                  XCTAssertTrue([jsonDoc isEqualToDictionary:aRev.body], @"JSON %@. NSError %@",
                                jsonDoc, error);
              }
          }
...
- (void)testDeleteNonExistingDoc
{
    ...
    NSDictionary *modifiedCount = @{
        @"docs" : [NSNumber numberWithInt:objectCount],
        @"revs" : [NSNumber numberWithInt:objectCount]
    };
    [self.dbutil checkTableRowCount:initialRowCount modifiedBy:modifiedCount];
    ...
    XCTAssertTrue(error.code == 404, @"Error was not a 404. Found %ld", error.code);
    XCTAssertNil(aRev,
                 @"CDTDocumentRevision should be nil after getting document that doesn't exist");
...
- (void)testDeleteDocumentUsingIdHasMultipleLeafNodesInTree
{
    ...
    TD_Revision *converted =
        [[TD_Revision alloc] initWithDocID:rev.docId revID:rev.revId deleted:rev.deleted];
    converted.body = [[TD_Body alloc] initWithProperties:body];

    TDStatus status;

    TD_Revision *new = [
        self.datastore.database putRevision : converted prevRevisionID : rev
            .revId allowConflict : YES status : &status
    ];
