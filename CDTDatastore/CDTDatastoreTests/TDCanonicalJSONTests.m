...
- (void)roundtrip:(id)obj
{
    ...
    NSError* error;
    id reconstituted = [NSJSONSerialization JSONObjectWithData:json
                                                       options:NSJSONReadingAllowFragments
                                                         error:&error];
    XCTAssertNotNil(reconstituted, @"Canonical JSON `%@` was unparseable: %@",
                    [json my_UTF8ToString], error);
    XCTAssertEqualObjects(reconstituted, obj,
                          @"Canonical JSON object and reconstructed objec were not equal in %s",
                          __PRETTY_FUNCTION__);
}
...
- (void)roundtripFloat:(double)n
{
    ...
    id reconstituted = [NSJSONSerialization JSONObjectWithData:json
                                                       options:NSJSONReadingAllowFragments
                                                         error:&error];
    XCTAssertNotNil(reconstituted, @"`%@` was unparseable: %@", [json my_UTF8ToString], error);
...
- (void)testEncoding
{
    XCTAssertEqualObjects([TDCanonicalJSON canonicalString:$true], @"true",
                          @"Canonical JSON $true is not \"true\" in %s", __PRETTY_FUNCTION__);
    XCTAssertEqualObjects([TDCanonicalJSON canonicalString:$false], @"false",
                          @"Canonical JSON $false is not \"false\" in %s", __PRETTY_FUNCTION__);
    XCTAssertEqualObjects([TDCanonicalJSON canonicalString:$null], @"null",
                          @"Canonical JSON $null is not \"null\" in %s", __PRETTY_FUNCTION__);
}
