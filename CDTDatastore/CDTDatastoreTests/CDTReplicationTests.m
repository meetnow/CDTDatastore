...
{
    XCTAssertNil([[CDTReplicatorFactory alloc] initWithDatastoreManager:nil],
                 @"Replication factory should be nil");
}
...
- (void)testDictionaryForPullReplicationDocument
{
    ...
    TDReplicator *tdreplicator =
        [replicatorManager createReplicatorWithProperties:pullDict error:&error];
    XCTAssertEqualObjects([tdreplicator class], [TDPuller class], @"Wrong Type of TDReplicator. %@",
                          error);
}
...
- (void)testDictionaryForPushReplicationDocument
{
    ...
    TDReplicatorManager *replicatorManager =
        [[TDReplicatorManager alloc] initWithDatabaseManager:self.factory.manager];
    TDReplicator *tdreplicator =
        [replicatorManager createReplicatorWithProperties:pushDict error:&error];
    XCTAssertEqualObjects([tdreplicator class], [TDPusher class], @"Wrong Type of TDReplicator. %@",
                          error);
}
...
- (void)testCreatePushReplicationWithFilter
{
    ...
    TDReplicatorManager *replicatorManager =
        [[TDReplicatorManager alloc] initWithDatabaseManager:self.factory.manager];
    TDReplicator *tdreplicator =
        [replicatorManager createReplicatorWithProperties:pushDoc error:&error];
    XCTAssertEqualObjects([tdreplicator class], [TDPusher class], @"Wrong Type of TDReplicator. %@",
                          error);
}
...
- (void)urlTestExpectTrue:(Class)prClass url:(NSURL *)url
{
...
- (void)urlTestExpectFalse:(Class)prClass url:(NSURL *)url withErrorCode:(NSInteger)code
{
    NSError *error = nil;
    CDTAbstractReplication *pr = [self buildReplicationObject:prClass remoteUrl:url];
...
- (void)runUrlTestFor:(Class)prClass
{
    ...
    [self urlTestExpectTrue:prClass
                        url:[NSURL
                                URLWithString:@"http://adam:pass@myaccount.cloudant.com:5000/foo"]];
    [self urlTestExpectTrue:prClass
    ...
    [self
        urlTestExpectTrue:prClass
                      url:[NSURL URLWithString:@"https://myaccount.cloudant.com/foo%2Fbar%2Fbam"]];
    [self urlTestExpectTrue:prClass
                        url:[NSURL URLWithString:
                                       @"https://myaccount.cloudant.com:5000/foo%2Fbar%2Fbam"]];
    [self urlTestExpectTrue:prClass
                        url:[NSURL URLWithString:@"https://adam:pass@myaccount.cloudant.com:5000/"
                                                 @"foo%2Fbar%2Fbam"]];
    [self
        urlTestExpectTrue:prClass
                      url:[NSURL
                              URLWithString:
                                  @"http://adam:pass@myaccount.cloudant.com:5000/foo%2Fbar%2Fbam"]];
    ...
    [self urlTestExpectTrue:prClass url:[NSURL URLWithString:@"https://someurl.com/foo/bar/bam"]];
