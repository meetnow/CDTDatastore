  ...
  describe(@"when executing a text search", ^{
    ...
    beforeEach(^{
      ...
      rev.body = @{
          ...
          @"age" : @12,
          @"pet" : @"cat",
          @"comment" : @"He lives in Bristol, UK "
      ...
      rev.body = @{
          ...
          @"age" : @34,
          @"pet" : @"dog",
          @"comment" : @"He lives in a van down "
                       @"by the river in "
                       @"Bristol."
      };
      [ds createDocumentFromRevision:rev error:nil];
    ...
    it(@"can perform a search consisting of a single "
       @"text clause",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"can perform a phrase search", ^{
      expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"]).toNot.beNil();
    ...
    it(@"can perform a search containing an "
       @"apostrophe",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"can perform a search consisting of a single "
       @"text clause with a sort",
       ^{
         expect([im ensureIndexed:@[ @"name", @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"can perform a compound AND query search "
       @"containing a text clause",
       ^{
         expect([im ensureIndexed:@[ @"name" ] withName:@"basic"]).toNot.beNil();
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();

         NSDictionary *query = @{ @"name" : @"mike", @"$text" : @{@"$search" : @"best friend"} };
         CDTQResultSet *result = [im find:query];
         expect(result.documentIds).to.containsInAnyOrder(@[ @"mike12" ]);
       });

    it(@"can perform a compound OR query search "
       @"containing a text clause",
       ^{
         expect([im ensureIndexed:@[ @"name" ] withName:@"basic"]).toNot.beNil();
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
         ...
         NSDictionary *query =
             @{ @"$or" : @[ @{@"name" : @"mike"}, @{@"$text" : @{@"$search" : @"best friend"}} ] };
         CDTQResultSet *result = [im find:query];
         expect(result.documentIds)
             .to.containsInAnyOrder(@[ @"mike12", @"mike34", @"mike72", @"fred12" ]);
       });

    it(@"returns nil for a text search without a "
       @"text index",
       ^{
         expect([im ensureIndexed:@[ @"name" ] withName:@"basic"]).toNot.beNil();
    ...
    it(@"returns nil for query containing text "
       @"clause when a needed json index is missing",
       ^{
         // All fields in a TEXT index only apply to
         // the text search portion of any query.
         // So even though "name" exists in the text
         // index, the clause that { "name" : "mike" }
         // expects a JSON index that contains the
         // "name" field.  Since, this query includes
         // a
         // text search clause then all clauses of the
         // query must be satisfied by existing
         // indexes.
         expect([im ensureIndexed:@[ @"name", @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"can perform a text search containing "
       @"non-ascii values",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"returns empty result set for unmatched "
       @"phrase search",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"returns correct result set for "
       @"non-contiguous word search",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();

         // The search predicate "Remus Romulus"
         // normalizes to "Remus AND Romulus" in
         // SQLite
         NSDictionary *query = @{ @"$text" : @{@"$search" : @"remus romulus"} };
         CDTQResultSet *result = [im find:query];
    ...
    it(@"can perform text search using enhanced "
       @"query syntax OR operator",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();

         // Enhanced Query Syntax - logical operators
         // must be uppercase otherwise they will
    ...
    it(@"can perform text search using enhanced "
       ...
       ^{
         ...
         // must be uppercase otherwise they will
         // be treated as a search token
         // - NOT operator only works between tokens
         // as in (token1 NOT token2)
    ...
    it(@"can perform text search using enhanced "
       @"query syntax with parentheses",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();

         // Parentheses are used to override SQLite
         // enhanced query syntax operator precedence
    ...
    it(@"can perform text search using NEAR operator", ^{
      ...
      // NEAR provides the ability to search for
      // terms/phrases in proximity to each other
      // - By specifying a value for NEAR as in
      // NEAR/2 you can define the range of
      // proximity.
    ...
    it(@"is case insensitive when using the default "
       @"tokenizer",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"treats non-string field as a string when "
       @"performing a text search",
       ^{
         expect([im ensureIndexed:@[ @"age" ] withName:@"basic_text" type:@"text"]).toNot.beNil();
    ...
    it(@"returns nil when text search criteria is "
       @"not a string",
       ^{
         expect([im ensureIndexed:@[ @"age" ] withName:@"basic_text" type:@"text"]).toNot.beNil();
    ...
    it(@"can perform a text search across multiple "
       @"fields",
       ^{
         expect([im ensureIndexed:@[ @"name", @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();

         // Will find fred12 and fred34 as well as
         // mike12 since Fred is also mentioned
         // in mike12's comment
    ...
    it(@"can perform a text search targeting "
       @"specific fields",
       ^{
         expect([im ensureIndexed:@[ @"name", @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();

         // Will only find fred12 since he is the only
         // named fred who's comment
    ...
    it(@"can perform a text search using prefix "
       @"searches",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"retuns empty result set when missing "
       @"wildcards in prefix searches",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
    ...
    it(@"can perform a text search using the Porter "
       @"tokenizer stemmer",
       ^{
    ...
    it(@"returns empty result set when using default "
       @"tokenizer stemmer",
       ^{
         expect([im ensureIndexed:@[ @"comment" ] withName:@"basic_text" type:@"text"])
             .toNot.beNil();
