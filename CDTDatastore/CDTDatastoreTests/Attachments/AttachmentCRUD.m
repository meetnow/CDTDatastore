...
- (BOOL)attachmentExists:(NSString *)filename
{
    ...
    NSString *attachmentsPath =
        [self.factoryPath stringByAppendingPathComponent:@"test attachments"];
...
- (void)testDocumentRevisionFactoryWithAttachmentDataIncluded
{
    ...
    NSDictionary *dict = @{
        ...
        @"_attachments" : @{
            @"bonsai-boston.jpg" : @{
                @"stub" : [NSNumber numberWithBool:NO],
    ...
    XCTAssertEqual([rev.attachments count], (NSUInteger)1,
                   @"Attachment count is wrong, expected 1 actual %d", [rev.attachments count]);
}
...
- (void)testDocumentRevisionFactoryWithAttachmentDataExcluded
{
    ...
    NSDictionary *dict = @{
        ...
        @"_attachments" : @{
            @"bonsai-boston.jpg" : @{
                @"stub" : [NSNumber numberWithBool:YES],
                @"length" : [NSNumber numberWithLong:[encoded length]],
                @"digest" : @"thisisahashiswear1234",
                @"revpos" : [NSNumber numberWithInt:1],
                @"content_type" : @"image/jpeg",
    ...
    CDTDocumentRevision *rev =
        [CDTDocumentRevision createRevisionFromJson:dict forDocument:attachmentDir error:&error];
    ...
    XCTAssertEqual([rev.attachments count], (NSUInteger)1,
                   @"Attachment count is wrong, expected 1 actual %d", [rev.attachments count]);
    XCTAssertEqualObjects(
        data, [[rev.attachments objectForKey:@"bonsai-boston.jpg"] dataFromAttachmentContent],
        @"data was not the same");
}
...
- (void)testCreate
{
    ...
    CDTDocumentRevision *rev2 = [self.datastore updateDocumentFromRevision:document error:&error];
    ...
    CDTAttachment *attachment =
        [[CDTUnsavedDataAttachment alloc] initWithData:data name:attachmentName type:@"image/jpg"];

    document = [rev2 copy];
    document.attachments = @{attachment.name : attachment};

    CDTDocumentRevision *rev3 = [self.datastore updateDocumentFromRevision:document error:&error];
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      XCTAssertTrue(
          [dc checkDatabase:db table:@"attachments" hasRows:expectedRows error:&validationError],
...
- (void)testUpdatingDocumentRetainsAttachments
{
    ...
    XCTAssertEqual((NSUInteger)1, [attachments count], @"Wrong number of attachments");
    XCTAssertEqualObjects(savedAttachment.name, @"bonsai-boston", @"Attachment wasn't in document");
...
- (void)testMultipleAttachments
{
    ...
    CDTAttachment *imgAttachment = [[CDTUnsavedDataAttachment alloc] initWithData:imageData
                                                                             name:@"bonsai-boston"
                                                                             type:@"image/jpg"];
    CDTAttachment *txtAttachment =
        [[CDTUnsavedDataAttachment alloc] initWithData:txtData name:@"lorem" type:@"text/plain"];

    document = [rev copy];
    document.attachments =
        @{imgAttachment.name : imgAttachment, txtAttachment.name : txtAttachment};
    rev = [self.datastore updateDocumentFromRevision:document error:&error];
    ...
    CDTAttachment *txtAttachment2 =
        [[CDTUnsavedDataAttachment alloc] initWithData:txtData name:@"lorem2" type:@"text/plain"];
    document = [rev copy];
    ...
    NSArray *expected =
        @[ @[ @"bonsai-boston", imageData ], @[ @"lorem", txtData ], @[ @"lorem2", txtData ] ];
    ...
    XCTAssertTrue([self attachmentExists:filenameImage],
...
- (void)testAddAttachments
{
    ...
    XCTAssertEqual((NSUInteger)2, [revision.attachments count], @"Wrong number of attachments");
...
- (void)testDelete
{
    ...
    attachments =
        [self.datastore getDocumentWithId:rev2.docId rev:rev2.revId error:&error].attachments;
    XCTAssertEqual((NSUInteger)1, [attachments count], @"Wrong number of attachments");
...
- (void)testFileDeletedAfterAttachmentCreatedGivesNilStream
{
    ...
    XCTAssertTrue([fm removeItemAtPath:tempPath error:nil], @"File couldn't be deleted");
...
- (void)testNilAttachmentStream
{
    ...
    CDTNullAttachment *attachment =
        [[CDTNullAttachment alloc] initWithName:@"name" type:@"type" size:100];
    ...
    XCTAssertEqual((NSInteger)kTDStatusAttachmentStreamError, error.code,
...
- (void)testNilAttachmentStreamWithGoodAttachmentStream
{
    ...
    [self.dbutil.queue inDatabase:^(FMDatabase *db) {
      ...
      NSArray *expectedRows = @[ @[ @"sequence" ], @[ @1 ] ];
...
- (void)testRetriveAttachmentsViaAllDocuments
{
    ...
    for (CDTDocumentRevision *revision in allDocuuments) {
        XCTAssertTrue([revision.attachments count] == 1, @"Attachment count is %d not 1",
...
- (void)testRetriveAttachmentsViaAllDocumentsById
{
    ...
    XCTAssertTrue([allDocuuments count] == 1, @"Unexpected number of documents 1 expected got %d",
                  [allDocuuments count]);
...
- (NSString *)tempFileName
{
    ...
    NSString *fileName = [NSString
        stringWithFormat:@"%@_%@", [[NSProcessInfo processInfo] globallyUniqueString], @"file.txt"];
