...
SpecBegin(CDTQValueExtractor)
    ...
    describe(@"from revision", ^{
      ...
      it(@"gets _rev", ^{
        NSObject *value = [CDTQValueExtractor extractValueForFieldName:@"_rev" fromRevision:rev];
        expect(value).to.equal(@"qweqeqwewqe");
      });

      it(@"gets name", ^{
        NSObject *value = [CDTQValueExtractor extractValueForFieldName:@"name" fromRevision:rev];
...
describe(@"when extracting single fields", ^{
  ...
  it(@"returns value for single field depth", ^{
    NSObject *v =
        [CDTQValueExtractor extractValueForFieldName:@"name" fromDictionary:@{
            @"name" : @"mike"
        }];
