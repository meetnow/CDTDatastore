                      ...
                      withAttachment:(BOOL)attach
{
    ...
    TD_Revision *revision = [[TD_Revision alloc] initWithDocID:rev1.docId revID:nil deleted:NO];
    revision.body = tdbody;

    TDStatus status;
    TD_Revision *td_rev =
        [tdstore putRevision:revision prevRevisionID:rev1.revId allowConflict:YES status:&status];
    ...
    revision = [[TD_Revision alloc] initWithDocID:rev1.docId revID:nil deleted:YES];
    revision.body = tdbody;
 ...
 */
- (void)addConflictingDocumentWithId:(NSString *)anId toDatastore:(CDTDatastore *)datastore
{
...
- (void)testResolveConflictWithMutableRevisionNoParent
{
    ...
    for (NSString *docId in conflictedDocs) {
        @try {
            NSError *error;
            [self.datastore resolveConflictsForDocument:docId resolver:myResolver error:&error];
...
- (void)testResolveConflictWithMutableRevisionWithAttachment
{
    ...
    //    ----- 2-c (seq 5, deleted = 1)
    //    /
    //    1-a (seq 1) --- 2-a (seq 2, attachment here) --- 3-a (seq 3, attachment here) --- 4-b (seq
    //    6, deleted=1)
    //    \
    //    ---- 2-b (seq 4)
    //
    ...
    XCTAssertEqual([TD_Revision generationFromRevID:rev.revId],
                   [TD_Revision generationFromRevID:myResolver.selectedParent.revId] + 1,
                   @"Unexpected RevId: %@", rev.revId);
    XCTAssertTrue([rev.body isEqualToDictionary:expectedBody], @"Unexpected document: %@",
                  rev.body);
        ...
        switch ([TD_Revision generationFromRevID:aRev.revId]) {
            case 2:
                XCTAssertEqualObjects(aRev.body,
                                      @{ @"foo2.b" : @"bar2.b" }, @"unexpected document: %@",
                                      aRev.body);
                break;

            case 3:
                XCTAssertEqualObjects(aRev.body,
                                      @{ @"foo3.a" : @"bar3.a" }, @"unexpected document: %@",
                                      aRev.body);
                break;
...
- (void)testCreateMultipleConflictingDocuments
{
    ...
    CDTDocumentRevision *rev = [self addNonConflictingDocumentWithBody:@{
        @"conflict" : @"no way!"
    }
    ...
    CDTDocumentRevision *rev2 = [self addNonConflictingDocumentWithBody:@{
        @"conflict" : @"no way!"
    }
                                                            toDatastore:self.datastore];

    NSSet *foundConflictedDocIds = [NSSet setWithArray:[self.datastore getConflictedDocumentIds]];
    XCTAssertTrue([foundConflictedDocIds isEqualToSet:setOfConflictedDocIds], @"foundSet: %@",
                  foundConflictedDocIds);

    XCTAssertFalse([foundConflictedDocIds containsObject:rev.docId],
                   @"conflicts set (%@) contained non-conflicting doc %@", foundConflictedDocIds,
                   rev.docId);
...
{
    // add a non-conflicting document
    [self addNonConflictingDocumentWithBody:@{
        @"conflict" : @"no way!"
    }
                                toDatastore:self.datastore];
...
- (void)testResolveSubset
{
    ...
    CDTTestParticularDocBiggestResolver *myResolver =
        [[CDTTestParticularDocBiggestResolver alloc] initWithDocsToResolve:resolvedDocs];
...
- (void)testResolveConflictWithAttachmentWithBiggestRev
{
    ...
    XCTAssertTrue([rev.attachments count] == 1,
                  @"Unexpected number of attachments. expected %d, actual %d", 1,
...
- (void)testResolveConflictWithAttachmentForRev2b
{
    ...
    CDTTestSpecificJSONDocumentResolver *myResolver =
        [[CDTTestSpecificJSONDocumentResolver alloc] initWithDictionary:@{
            @"foo2.b" : @"bar2.b"
        }];
    ...
    XCTAssertEqual([TD_Revision generationFromRevID:rev.revId], (unsigned)2,
                   @"Unexpected RevId: %@", rev.revId);
    XCTAssertTrue([rev.body isEqualToDictionary:@{
                      @"foo2.b" : @"bar2.b"
                  }],
                  @"Unexpected document: %@", rev.body);

    NSArray *attachments = [self.datastore attachmentsForRev:rev error:nil];
