...
- (void)printFMResult:(FMResultSet *)result ignorecolumns:(NSSet *)ignored
{
    for (int i = 0; i < [result columnCount]; i++) {
        ...
        if ([ignored member:columnName]) continue;
        ...
        if ([columnName isEqualToString:@"json"]) {
            ...
            if (jsonDoc)
                ...
                resultString = [NSString stringWithFormat:@"object type: %@. description: %@",
                                                          [[result objectForColumnIndex:i] class],
                                                          [result dataForColumnIndex:i]];
        }
...
{
    NSString *sql =
        [NSString stringWithFormat:@"select revs.doc_id as doc_id, docid, revid, sequence, parent, "
                                   @"current, deleted, json from revs, docs where revs.doc_id = "
                                   @"docs.doc_id order by docid, sequence"];
    [self printResultsForQuery:sql];
}
...
- (NSSet *)sqlTables
{
    if (_sqlTables) return _sqlTables;
                ...
                modifiedBy:(NSDictionary *)modifiedRowCount
{
    for (NSString *table in initialRowCount) {
        ...
        if ([modifiedRowCount[table] respondsToSelector:@selector(integerValue)])
            expectCount += [modifiedRowCount[table]
                integerValue];  // we expect there to be one new row in the modifiedTables
