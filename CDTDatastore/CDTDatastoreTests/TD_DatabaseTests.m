...
- (void)testRevisionDictionary
{
    ...
    XCTAssertEqualObjects(makeRevisionHistoryDict(revs), $dict({
                              @"ids", @[ @"jkl", @"ghi", @"def" ]
                          }, { @"start", @4 }), @"4-3-2 revs failed in %s", __PRETTY_FUNCTION__);

    revs = @[ [self mkrev:@"4-jkl"], [self mkrev:@"2-def"] ];
    XCTAssertEqualObjects(makeRevisionHistoryDict(revs), $dict({ @"ids", @[ @"4-jkl", @"2-def" ] }),
                          @"4-2 revs failed in %s", __PRETTY_FUNCTION__);

    revs = @[ [self mkrev:@"12345"], [self mkrev:@"6789"] ];
    XCTAssertEqualObjects(makeRevisionHistoryDict(revs), $dict({ @"ids", @[ @"12345", @"6789" ] }),
                          @"12345-6789 revs failed in %s", __PRETTY_FUNCTION__);
}
