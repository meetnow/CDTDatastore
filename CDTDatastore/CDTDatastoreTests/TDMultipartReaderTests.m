...
- (void)startedPart:(NSDictionary*)headers
{
    ...
    if (!_partList) _partList = [[NSMutableArray alloc] init];
    [_partList addObject:_currentPartData];
    if (!_headersList) _headersList = [[NSMutableArray alloc] init];
...
- (void)testBoundaryObject
{
    ...
    TDMultipartReader* reader =
        [[TDMultipartReader alloc] initWithContentType:@"multipart/related; boundary=\"BOUNDARY\""
                                              delegate:nil];
    XCTAssertEqualObjects(
        reader.boundary, [@"\r\n--BOUNDARY" dataUsingEncoding:NSUTF8StringEncoding],
        @"Quotation escaped Boundary objects not equal in %s", __PRETTY_FUNCTION__);

    reader = [[TDMultipartReader alloc] initWithContentType:@"multipart/related; boundary=BOUNDARY"
                                                   delegate:nil];
    XCTAssertEqualObjects(reader.boundary,
                          [@"\r\n--BOUNDARY" dataUsingEncoding:NSUTF8StringEncoding],
                          @"No quotation Boundary objects not equal in %s", __PRETTY_FUNCTION__);

    reader =
        [[TDMultipartReader alloc] initWithContentType:@"multipart/related; boundary=\"BOUNDARY"
                                              delegate:nil];
    XCTAssertNil(reader, @"TDMultipartReader not nil with improper initialization in %s",
                 __PRETTY_FUNCTION__);

    reader = [[TDMultipartReader alloc] initWithContentType:@"multipart/related;boundary=X"
                                                   delegate:nil];
    XCTAssertEqualObjects(reader.boundary, [@"\r\n--X" dataUsingEncoding:NSUTF8StringEncoding],
                          @"No quotation Arbitrary objects not equal in %s", __PRETTY_FUNCTION__);
}
...
- (void)testSimpleMultiPartReadWithDelegate
{
    ...
    NSArray* expectedParts = @[
        [@"part the first" dataUsingEncoding:NSUTF8StringEncoding],
        [@"2nd part" dataUsingEncoding:NSUTF8StringEncoding]
    ];
    NSArray* expectedHeaders = @[ $dict({ @"Foo", @"Bar" }, { @"Header", @"Val ue" }), $dict() ];
    ...
    for (NSUInteger chunkSize = 1; chunkSize <= mime.length; ++chunkSize) {
        ...
        TDMultipartReader* reader = [[TDMultipartReader alloc]
            initWithContentType:@"multipart/related; boundary=\"BOUNDARY\""
                       delegate:delegate];
        XCTAssertFalse(reader.finished, @"Premature finished reading data in %s",
                       __PRETTY_FUNCTION__);

        NSRange r = {0, 0};
        do {
            XCTAssertTrue(r.location < mime.length, @"Parser didn't stop at end in %s",
                          __PRETTY_FUNCTION__);
            r.length = MIN(chunkSize, mime.length - r.location);
            [reader appendData:[mime subdataWithRange:r]];
            XCTAssertTrue(!reader.error, @"Reader got a parse error: %@ in %s", reader.error,
                          __PRETTY_FUNCTION__);
            r.location += chunkSize;
        } while (!reader.finished);
        XCTAssertEqualObjects(delegate.partList, expectedParts,
                              @"Unexpected part in Delegate in %s", __PRETTY_FUNCTION__);
        XCTAssertEqualObjects(delegate.headerList, expectedHeaders,
                              @"Unexpected Headers in Delegate in %s", __PRETTY_FUNCTION__);
    }
}
