...
- (void)testMultipartDownloader
{
    ...
    if (!$equal(NSUserName(), @"snej")) return;
    ...
    TD_Database* db = [TD_Database
              createEmptyDBAtPath:[NSTemporaryDirectory()
                                      stringByAppendingPathComponent:@"TDMultipartDownloader"]
        withEncryptionKeyProvider:provider];

    // NSString* urlStr =
    // @"http://127.0.0.1:5984/demo-shopping-attachments/2F9078DF-3C72-44C2-8332-B07B3A29FFE4"
    ...
    [[[TDMultipartDownloader alloc]
        initWithSession:[[CDTURLSession alloc] init]
                    URL:url
               database:db
         requestHeaders:nil
           onCompletion:^(id result, NSError* error) {
             XCTAssertNil(error,
                          @"NSError is not nil after alloc init of TDMultipartDownloader in %s",
                          __PRETTY_FUNCTION__);
             ...
             XCTAssertTrue(attachments.count >= 1, @"attachments.count >= 1 fails in %s",
                           __PRETTY_FUNCTION__);
             XCTAssertEqual([db blobCount], 0u, @"db.attachmentStore.count is not 0u in %s",
                            __PRETTY_FUNCTION__);
             for (NSDictionary* attachment in attachments.allValues) {
                 ...
                 __block BOOL wasBlobInstalled = NO;
                 [db.fmdbQueue inDatabase:^(FMDatabase* db) {
                   wasBlobInstalled = [writer installWithDatabase:db];
                 }];
                 XCTAssertTrue(wasBlobInstalled, @"TDBlobStoreWriter install returned NO in %s",
                               __PRETTY_FUNCTION__);

                 id<CDTBlobReader> blob = [db blobForKey:writer.blobKey];
                 NSData* data = [blob dataWithError:nil];
                 //              NSLog(@"Found %u bytes of data for attachment %@",
                 //              (unsigned)blob.length, attachment);
                 NSNumber* lengthObj = attachment[@"encoded_length"] ?: attachment[@"length"];
                 XCTAssertEqual(data.length, [lengthObj unsignedLongLongValue],
                                @"blob length and object length are not equal in %s",
                                __PRETTY_FUNCTION__);
                 XCTAssertEqual(writer.length, data.length,
                                @"writer length and blog length are not equal in %s",
                                __PRETTY_FUNCTION__);
             }
             XCTAssertEqual([db blobCount], attachments.count,
                            @"db.attachmentStore.count and attachments.count are not equal in %s",
                            __PRETTY_FUNCTION__);
             done = YES;
           }] start];
