...
+ (NSString *)userAgentHeader
{
    ...
    platform = @"Mac OS X";
#endif
    return [NSString stringWithFormat:@"CloudantSync/%@ (%@ %@)", [CDTDatastore versionString],
                                      platform, version];
}
...
{
    return [NSString stringWithFormat:@"%@[%@ %@]", [self class], _request.HTTPMethod,
                                      TDCleanURLtoString(_request.URL)];
}
...
- (bool)retryWithCredential { return false; }
...
{
    CDTLogVerbose(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@: Got %lu bytes", self,
...
- (void)requestDidError:(NSError *)error
{
    if (!(_dontLog404 && error.code == kTDStatusNotFound &&
        ...
        CDTLogVerbose(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@: Got error. domain %@, code %@", self,
...
- (void)receivedData:(NSData *)data
{
    ...
    if (_jsonBuffer.length > 0) {
        ...
        if (!result) {
            CDTLogWarn(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@: %@ %@ returned unparseable data '%@'",
                       self, _request.HTTPMethod, TDCleanURLtoString(_request.URL),
