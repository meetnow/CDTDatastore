...
- (void)databaseClosingOnMyThread { [self stop]; }
- (NSString*)description { return $sprintf(@"%@ [%@]", [self class], TDCleanURLtoString(_remote)); }
...
- (void)postProgressChanged
{
    CDTLogWarn(CDTREPLICATION_LOG_CONTEXT,
               ...
               (unsigned)_changesProcessed, (unsigned)_changesTotal, _active,
               (unsigned)_batcher.count, _asyncTaskCount, _online);
    NSNotification* n =
...
- (void)start
{
    ...
    _replicatorThread =
        [[NSThread alloc] initWithTarget:self selector:@selector(runReplicatorThread) object:nil];
    CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"Starting TDReplicator thread %@ ...",
               _replicatorThread);
    [_replicatorThread start];

    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(databaseWasDeleted:)
...
- (void)startReplicatorTasks
{
    ...
    _batcher = [[TDBatcher alloc]
        initWithCapacity:kInboxCapacity
                   delay:kProcessDelay
               processor:^(NSArray* inbox) {
                 CDTLogWarn(CDTREPLICATION_LOG_CONTEXT,
                            @"*** %@: BEGIN processInbox (%u sequences)", self,
                            (unsigned)inbox.count);
                 TD_RevisionList* revs = [[TD_RevisionList alloc] initWithArray:inbox];
                 [self processInbox:revs];
                 CDTLogWarn(CDTREPLICATION_LOG_CONTEXT,
                            @"*** %@: END processInbox (lastSequence=%@)", self, _lastSequence);
                 [self updateActive];
               }];
    ...
    if (!_authorizer) {
        ...
        if (_authorizer)
            CDTLogWarn(CDTREPLICATION_LOG_CONTEXT, @"%@: Found credential, using %@", self,
                       _authorizer);
    }
...
- (BOOL)threadExecuting { return [self.replicatorThread isExecuting]; }
- (BOOL)threadFinished { return [self.replicatorThread isFinished]; }
- (BOOL)threadCanceled { return [self.replicatorThread isCancelled]; }
...
- (void)retryIfReady
{
    ...
    if (_online) {
        CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@ RETRYING, to transfer missed revisions...",
                   self);
        _revisionsFailed = 0;
        [NSObject cancelPreviousPerformRequestsWithTarget:self
                                                 selector:@selector(retryIfReady)
    ...
    [self sendAsyncRequest:@"GET"
              ...
              onCompletion:^(id result, NSError* error) {
                if (error) {
                    ...
                    NSString* username =
                        $castIf(NSString, [[result objectForKey:@"userCtx"] objectForKey:@"name"]);
                    if (username) {
                        CDTLogInfo(CDTREPLICATION_LOG_CONTEXT,
                                   @"%@: Active session, logged in as '%@'", self, username);
                        [self fetchRemoteCheckpointDoc];
                    } else {
                        [self login];
                    }
...
- (void)login
{
    ...
    CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@: Logging in with %@ at %@ ...", self,
               _authorizer.class, loginPath);
    [self asyncTaskStarted];
         ...
         requestHeaders:self.requestHeaders
           onCompletion:^(id result, NSError* error) {
             ...
             if (auth && auth != _authorizer && error.code != 401) {
                 CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@: Updated to %@", self, auth);
                 _authorizer = auth;
             }
...
- (NSString*)remoteCheckpointDocID
{
    NSMutableDictionary* spec =
        $mdict({ @"localUUID", _db.privateUUID }, { @"remoteURL", _remote.absoluteString }, {
            @"push", @(self.isPush)
        }, { @"filter", _filterName }, { @"filterParams", _filterParameters });
        ...
        sendAsyncRequest:@"GET"
            ...
            onCompletion:^(id response, NSError* error) {
              ...
              if (error && error.code != kTDStatusNotFound) {
                  CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@: Error fetching last sequence: %@",
                  ...
                  if ($equal(remoteLastSequence, localLastSequence)) {
                      ...
                      CDTLogInfo(CDTREPLICATION_LOG_CONTEXT,
                                 @"%@: Replicating from lastSequence=%@", self, _lastSequence);
                  } else {
                      CDTLogInfo(
...
- (void)saveLastSequence
{
    ...
    [self sendAsyncRequest:@"PUT"
              ...
              onCompletion:^(id response, NSError* error) {
                ...
                if (error) {
                    CDTLogWarn(CDTREPLICATION_LOG_CONTEXT,
                               @"%@: Unable to save remote checkpoint: %@", self, error);
                ...
                CDTLogVerbose(CDTREPLICATION_LOG_CONTEXT,
                              @"PUT last sequence %@ to checkpoint doc response: %@", _lastSequence,
                              response);
                [self asyncTasksFinished:1];
