    ...
    [_fmdbQueue inDatabase:^(FMDatabase* db) {
      ...
      @try {
          ...
          if (keyData.length != sizeof(TDBlobKey)) {
              CDTLogWarn(CDTDATASTORE_LOG_CONTEXT,
                         @"%@: Attachment %lld.'%@' has bogus key size %u", self, sequence,
                         filename, (unsigned)keyData.length);
    ...
    do {
        ...
        if (options & kTDIncludeAttachments) {
            ...
            if ((options & kTDBigAttachmentsFollow) && effectiveLength >= kBigAttachmentLength) {
                ...
                id<CDTBlobReader> blob =
                    [_attachments blobForKey:*(TDBlobKey*)keyData.bytes withDatabase:db];
                    ...
                    withBlock:^NSDictionary*(NSString* name, NSDictionary* attachment) {
                      ...
                      if (stubItOut) {
                          ...
                          CDTLogVerbose(CDTDATASTORE_LOG_CONTEXT,
                                        @"Stubbed out attachment %@/'%@': revpos %d < %d", rev,
                                        name, revPos, minRevPos);
                      } else if (addFollows) {
                      ...
                      inDatabase:(FMDatabase*)db
{
    *outStatus = kTDStatusBadAttachment;
    ...
    if (body) {
        ...
        attachments[filename] = $dict({ @"digest", digest }, { @"length", @(body.length) }, {
            @"follows", $true
