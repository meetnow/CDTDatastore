...
- (BOOL)start
{
    ...
    if (_unauthResponse && _credential) {
        ...
        if (!password) {
            ...
            CDTLogInfo(CDTREPLICATION_LOG_CONTEXT,
                       @"Huh, couldn't get password of %@; trying again", _credential);
            _credential =
        ...
        if (password) {
            ...
            CDTLogWarn(CDTREPLICATION_LOG_CONTEXT, @"%@: Unable to get password of credential %@",
                       self, _credential);
            _credential = nil;
    ...
    CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@: Started... <%@>", self,
               TDCleanURLtoString(self.changesFeedURL));
    return YES;
}
...
- (BOOL)handleInvalidResponse:(NSData*)body
{
    ...
    CDTLogWarn(CDTREPLICATION_LOG_CONTEXT,
               @"%@: Longpoll connection closed (by proxy?) after %.1f sec", self, elapsed);
...
- (void)stream:(NSStream*)stream handleEvent:(NSStreamEvent)eventCode
{
    ...
    switch (eventCode) {
        ...
        default:
            CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@: Event %lx on %@", self, (long)eventCode,
                       stream);
            break;
    }
}
