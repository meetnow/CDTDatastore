        ...
        if (authHeader) {
            if ([requestHeadersKeys containsObject:@"Authorization"]) {
                CDTLogWarn(CDTREPLICATION_LOG_CONTEXT,
                           @"%@ Overwriting 'Authorization' header with "
                           @"value %@",
                           self, authHeader);
            }
          ...
          (void (^)(NSURLSessionAuthChallengeDisposition, NSURLCredential *))completionHandler
{
    ...
    CDTLogVerbose(CDTREPLICATION_LOG_CONTEXT,
                  @"Got challenge for %@: method=%@, proposed=%@, err=%@", [self class], authMethod,
                  challenge.proposedCredential, challenge.error);
    ...
    if ($equal(authMethod, NSURLAuthenticationMethodHTTPBasic)) {
        ...
        if ([TDRemoteRequest checkTrust:trust forHost:space.host]) {
            CDTLogVerbose(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@ useCredential for trust: %@", self,
                          trust);
...
- (void)receivedResponse:(NSURLResponse *)response
{
    ...
    CDTLogVerbose(CDTREPLICATION_LOG_CONTEXT, @"%@: didReceiveResponse, status %ld", [self class],
                  (long)status);
    ...
    if (TDStatusIsError(status)) {
        ...
        if (status == 401 || status == 407) {
            ...
            CDTLogError(
                ...
                @"%@: HTTP auth failed; sent Authorization: %@  ;  got WWW-Authenticate: %@",
                [self class], authorization, authResponse);
...
{
    CDTLogVerbose(CDTREPLICATION_LOG_CONTEXT, @"%@: didReceiveData: %ld bytes", [self class],
...
- (void)failedWithError:(NSError *)error { [self retryOrError:error]; }
- (BOOL)receivedDataBeginsCorrectly
{
    ...
    if (!match) {
        CDTLogError(CDTREPLICATION_LOG_CONTEXT, @"%@: Unparseable response from %@. Did not find "
                                                @"start of the expected response: %@",
                    self, TDCleanURLtoString(self.request.URL), prefixString);
    }
