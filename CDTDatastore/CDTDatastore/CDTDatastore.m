...
- (NSArray *)getAllDocumentIds
{
    ...
    struct TDQueryOptions query = {
        .limit = UINT_MAX, .inclusiveEnd = YES, .skip = 0, .descending = NO, .includeDocs = NO};
...
- (BOOL)ensureDatabaseOpen { return [_database openWithEncryptionKeyProvider:self.keyProvider]; }
                                              ...
                                              error:(NSError *__autoreleasing *)error
{
    ...
    [self.database.fmdbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
      ...
      TDStatus status;
      TD_Revision *new = [
          datastore.database putRevision : converted prevRevisionID : nil allowConflict : NO
                                                ...
                                                error:(NSError *__autoreleasing *)error
{
    ...
    TDStatus status;
    TD_Revision *new = [
        self.database putRevision : revision prevRevisionID : prevRev allowConflict : NO
            status : &status database : db
    ];
    if (TDStatusIsError(status)) {
        *error = TDStatusToNSError(status, nil);
                                              ...
                                              error:(NSError *__autoreleasing *)error
{
    ...
    TDStatus status;
    TD_Revision *new = [
        self.database putRevision : td_revision prevRevisionID : prevRevisionID allowConflict : NO
            status : &status
    ];
    if (TDStatusIsError(status)) {
        if (error) {
...
- (NSArray *)deleteDocumentWithId:(NSString *)docId error:(NSError *__autoreleasing *)error
{
    ...
    [self.database.fmdbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
      ...
      while ([result next]) {
          ...
          TDStatus status;
          TD_Revision *new = [
              weakself.database putRevision : td_revision prevRevisionID : revId allowConflict : NO
                  status : &status database : db
          ];
