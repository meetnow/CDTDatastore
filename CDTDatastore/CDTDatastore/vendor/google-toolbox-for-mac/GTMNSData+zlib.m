                               ...
                               useGzip:(BOOL)useGzip
{
    ...
    if ((retCode = deflateInit2(&strm, level, Z_DEFLATED, windowBits, memLevel,
        ...
        _GTMDevLog(@"Failed to init for deflate w/ level %d, error %d", level, retCode);
        return nil;
        // COV_NF_END
    }
    ...
    do {
        ...
        if ((retCode != Z_OK) && (retCode != Z_STREAM_END)) {
            ...
            // way to test it)
            _GTMDevLog(@"Error trying to deflate some of the payload, error %d", retCode);
            deflateEnd(&strm);
            return nil;
            // COV_NF_END
        }
...
+ (NSData *)gtm_dataByGzippingBytes:(const void *)bytes
                   ...
                   compressionLevel:(int)level
{
    return [self gtm_dataByCompressingBytes:bytes length:length compressionLevel:level useGzip:YES];
}  // gtm_dataByGzippingBytes:length:level:

+ (NSData *)gtm_dataByGzippingData:(NSData *)data compressionLevel:(int)level
{
    return [self gtm_dataByCompressingBytes:[data bytes]
                                     length:[data length]
...
+ (NSData *)gtm_dataByDeflatingBytes:(const void *)bytes length:(NSUInteger)length
{
    return [self gtm_dataByCompressingBytes:bytes
                                     length:length
                           compressionLevel:Z_DEFAULT_COMPRESSION
...
+ (NSData *)gtm_dataByDeflatingBytes:(const void *)bytes
                    ...
                    compressionLevel:(int)level
{
    return [self gtm_dataByCompressingBytes:bytes length:length compressionLevel:level useGzip:NO];
}  // gtm_dataByDeflatingBytes:length:level:

+ (NSData *)gtm_dataByDeflatingData:(NSData *)data compressionLevel:(int)level
{
    return [self gtm_dataByCompressingBytes:[data bytes]
                                     length:[data length]
                           compressionLevel:level
...
+ (NSData *)gtm_dataByInflatingBytes:(const void *)bytes length:(NSUInteger)length
{
    ...
    do {
        ...
        if ((retCode != Z_OK) && (retCode != Z_STREAM_END)) {
            _GTMDevLog(@"Error trying to inflate some of the payload, error %d", retCode);
            inflateEnd(&strm);
            return nil;
        }
        // collect what we got
