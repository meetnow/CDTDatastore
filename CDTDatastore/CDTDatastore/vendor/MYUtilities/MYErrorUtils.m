...
static NSError *MYMakeErrorV(int errorCode, NSString *domain, NSString *message, va_list args)
{
    ...
    NSDictionary *userInfo =
        [NSDictionary dictionaryWithObjectsAndKeys:message, NSLocalizedDescriptionKey, nil];
    [message release];
...
BOOL MYReturnError(NSError **outError, int errorCode, NSString *domain, NSString *messageFormat,
                   ...)
{
    if (errorCode) {
        if (outError) {
...
{
    if (t < 0x20202020 || t > 0x7e7e7e7e) return nil;
    union {
        OSType ostype;
        unsigned char ch[4];
    } buf;
    ...
    for (int i = 0; i < 4; i++)
        if (buf.ch[i] < 0x20 || buf.ch[i] > 0x7E) return nil;
...
static NSString *printableErrorCode(NSInteger code)
{
    if ((OSStatus)code < -99999)
        return $sprintf(@"%u",
                        (unsigned)code);  // CSSM errors are huge unsigned values > 0x80000000
    NSString *result = printableOSType((OSType)code);
    if (result) return result;            // CoreAudio errors are OSTypes (4-char strings)
...
static NSString *MYShortErrorDomainName(NSString *domain)
{
    if ([domain hasPrefix:@"kCFErrorDomain"])
        ...
        if ([domain hasPrefix:@"NS"]) domain = [domain substringFromIndex:2];
    }
    return domain;
}
...
NSString *MYErrorName(NSString *domain, NSInteger code)
{
    ...
    if ($equal(domain, NSPOSIXErrorDomain)) {
        ...
        if (name) {
            ...
            if ([result hasPrefix:@"Unknown error"]) result = nil;
        }
    }
        ...
        if (!result) {
            ...
            if (result) {
                ...
                if ([result hasPrefix:@"OSStatus "]) result = nil;  // just a generic message
            }
        }
#endif
    }
...
- (NSError *)my_errorByPrependingMessage:(NSString *)message
{
    if (message.length) {
        ...
        if (oldUserInfo) [userInfo addEntriesFromDictionary:oldUserInfo];
        NSString *desc = [oldUserInfo objectForKey:NSLocalizedDescriptionKey];
        if (desc) message = $sprintf(@"%@: %@", message, desc);
...
- (NSString *)my_nameOfCode { return MYErrorName(self.domain, self.code); }
@end
