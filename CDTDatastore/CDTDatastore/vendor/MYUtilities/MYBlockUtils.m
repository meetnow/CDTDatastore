...
/* This is sort of a kludge. This method only needs to be defined for blocks, but their class
 * (NSBlock) isn't public, and the only public base class is NSObject. */
@implementation NSObject (MYBlockUtils)

- (void)my_run_as_block { ((void (^)())self)(); }
@end
...
void MYAfterDelay(NSTimeInterval delay, void (^block)())
{
...
#endif
    {
        ...
        [block performSelector:@selector(my_run_as_block) withObject:nil afterDelay:delay];
    }
}
...
TestCase(MYAfterDelay)
{
    ...
    MYAfterDelayInModes(0.5, $array(NSRunLoopCommonModes), ^{
      fired = YES;
      NSLog(@"Fired!");
    });
    CAssert(!fired);
