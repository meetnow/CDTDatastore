    ...
    if (_docIDs) {
        if (_filterName) {
            CDTLogInfo(CDTREPLICATION_LOG_CONTEXT,
                       @"You can't set both a replication filter and "
            ...
            if (!docIDsParam || error) {
                CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"Illegal doc IDs %@, %@",
                           [_docIDs description], [error localizedDescription]);
            }
...
- (void)failedWithError:(NSError*)error
{
    ...
    if (TDMayBeTransientError(error)) {
        ...
        ++_retryCount;
        CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@: Connection error, retrying in %.1f sec: %@",
