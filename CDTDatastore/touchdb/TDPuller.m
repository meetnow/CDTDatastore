...
- (void)beginReplicating
{
    if (!_downloadsToInsert) {
        ...
        _downloadsToInsert = [[TDBatcher alloc] initWithCapacity:200
                                                           delay:1.0
...
- (void)startChangeTracker
{
    ...
    CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@ starting ChangeTracker: mode=%d, since=%@", self,
               mode, _lastSequence);
...
{
    CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@: Received %u changes", self,
...
- (void)pullRemoteRevision:(TD_Revision*)rev
{
    ...
    dl = [[TDMultipartDownloader alloc]
...
- (void)pullBulkRevisions:(NSArray*)bulkRevs
{
    ...
    CDTLogInfo(CDTREPLICATION_LOG_CONTEXT, @"%@ bulk-fetching %u remote revisions...", self,
               (unsigned)nRevs);
    CDTLogVerbose(CDTREPLICATION_LOG_CONTEXT, @"%@ bulk-fetching remote revisions: %@", self,
                  bulkRevs);
            ...
            @autoreleasepool {
                ...
                if (TDStatusIsError(status)) {
                    if (status == kTDStatusForbidden)
                        CDTLogInfo(CDTREPLICATION_LOG_CONTEXT,
                                   @"%@: Remote rev failed validation: %@", self, rev);
                    else {
                        CDTLogWarn(CDTREPLICATION_LOG_CONTEXT, @"%@ failed to write %@: status=%d",
                                   self, rev, status);
                        [self revisionFailed];
