...
{
    CDTLogInfo(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@: adding stream of unknown length: %@", self,
               stream);
...
#endif
    CDTLogInfo(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@: Opened input=%p, output=%p", self, _input,
               _output);
    [self opened];
    return _input;
}
...
- (NSInteger)read:(uint8_t*)buffer maxLength:(NSUInteger)len
{
    ...
    while (len > 0 && _currentInput) {
        ...
        CDTLogInfo(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@:     read %d bytes from %@", self,
                   (int)bytesRead, _currentInput);
        if (bytesRead > 0) {
...
- (BOOL)writeToOutput
{
    ...
    CDTLogInfo(CDTTD_REMOTE_REQUEST_CONTEXT,
               ...
               (int)bytesWritten, (unsigned)_bufferLength, _totalBytesWritten + bytesWritten,
               _length);
    if (bytesWritten <= 0) {
    ...
    switch (event) {
        ...
        case NSStreamEventHasSpaceAvailable:
            if (_input && _input.streamStatus < NSStreamStatusOpen) {
                ...
                CDTLogInfo(CDTTD_REMOTE_REQUEST_CONTEXT, @"%@:   Input isn't open; waiting...",
                           self);
