...
const TDQueryOptions kDefaultTDQueryOptions = {
    .limit = UINT_MAX, .inclusiveEnd = YES
          ...
          while (keepGoing) {
              @autoreleasepool {
                  ...
                  if (lastSequence > 0) {
                      ...
                      while ([r2 next]) {
                          ...
                          if (first) {
                              ...
                              [fmdb
                                  executeUpdate:@"DELETE FROM maps WHERE view_id=? AND sequence=?",
                                                @(_viewID), @(oldSequence)];
                              if (TDCompareRevIDs(oldRevID, revID) > 0) {
                                  ...
                                  json =
                                      [fmdb dataForQuery:@"SELECT json FROM revs WHERE sequence=?",
                                                         @(sequence)];
                              }
                          }
                      }
                  ...
                  NSDictionary* properties = [_db documentPropertiesFromJSON:json
                                                                  ...
                                                                  inDatabase:db];
                  if (!properties) {
                      CDTLogWarn(CDTTD_VIEW_CONTEXT, @"Failed to parse JSON of doc %@ rev %@",
                                 docID, revID);
                      continue;
                  }
      ...
      if (options->reduce || group) {
          ...
          while ([r next]) {
              @autoreleasepool {
                  ...
                  CDTLogVerbose(CDTTD_VIEW_CONTEXT,
                                @"Query %@: Found row with key=%@, value=%@, id=%@", _name,
                                toJSONString(key), toJSONString(value), toJSONString(docID));
...
- (NSMutableArray*)reducedQuery:(FMResultSet*)r group:(BOOL)group groupLevel:(unsigned)groupLevel
{
    ...
    while ([r next]) {
        @autoreleasepool {
            ...
            CDTLogVerbose(CDTTD_VIEW_CONTEXT, @"Query %@: Will reduce row with key=%@, value=%@",
