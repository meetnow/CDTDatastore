...
- (NSObject *)lastSequenceWithCheckpointID:(NSString *)checkpointID
{
    ...
    [_fmdbQueue inDatabase:^(FMDatabase *db) {
      lastSequenceJson =
          [db dataForQuery:@"SELECT last_sequence FROM replicators WHERE remote=?", checkpointID];

    }];
    if (lastSequenceJson != nil) {
        NSDictionary *lastSequence =
            [TDJSON JSONObjectWithData:lastSequenceJson options:0 error:nil];
