...
- (void)dealloc
{
    if (self.tdReplicator.running) {
        ...
        NSString *message = [NSString
            stringWithFormat:@"Object deallocated before completed "
...
- (NSString *)description
{
    ...
    NSString *fullinfo = [NSString
        stringWithFormat:@"CDTReplicator %@, source: %@, target: %@ "
                         @"filter name: %@, filter parameters %@, unique replication session "
                         @"ID: %@",
                         replicationType, source, target, self.tdReplicator.filterName,
...
- (void)replicatorStarted:(NSNotification *)n
{
    ...
    CDTReplicatorState oldState = strongSelf.state;
    @synchronized(strongSelf)
    {  // lock out other processes from changing state. strongSelf.state =
...
- (BOOL)threadFinished { return self.tdReplicator.threadFinished; }
- (BOOL)threadCanceled { return self.tdReplicator.threadCanceled; }
@end
