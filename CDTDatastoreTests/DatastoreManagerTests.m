...
- (void)testList5Datastores
{
    ...
    XCTAssertEqual((NSUInteger)5, [datastores count],
                   @"Wrong number of datastores returned, expected 5 got %d", [datastores count]);
...
- (void)testListDatastoresWithSlash
{
    ...
    XCTAssertEqual((NSUInteger)1, [datastores count],
                   @"Wrong number of datastores returned, expected 1 got %d", [datastores count]);
    XCTAssertEqualObjects(@"adatabase/withaslash", [datastores objectAtIndex:0],
...
- (void)testListEmptyDatastores
{
    ...
    XCTAssertEqual((NSUInteger)0, [datastores count],
                   @"Wrong number of datastores returned, expected 0 got %d", [datastores count]);
}
...
- (void)testSchema6ToSchema100Upgrade
{
    ...
    CDTDatastoreManager *customFactory =
        [[CDTDatastoreManager alloc] initWithDirectory:[dbPath stringByDeletingLastPathComponent]
                                                 error:&err];
    CDTDatastore *store =
        [customFactory datastoreNamed:[[dbPath lastPathComponent] stringByDeletingPathExtension]
                                error:&err];
