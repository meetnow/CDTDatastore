    ...
    sharedExamplesFor(@"queries with covering indexes", ^(NSDictionary* data) {
      ...
      describe(@"when executing queries", ^{
        ...
        it(@"can query over two string fields", ^{
          NSDictionary* query = @{ @"name" : @{@"$eq" : @"mike"}, @"pet" : @{@"$eq" : @"cat"} };
          CDTQResultSet* result = [im find:query];
          expect(result).toNot.beNil();
          expect(result.documentIds.count).to.equal(2);
        });
        ...
        context(@"when limiting and offsetting results", ^{
          ...
          it(@"returns appropriate results skip and large limit used", ^{
            NSDictionary* query = @{ @"name" : @{@"$eq" : @"mike"} };
            CDTQResultSet* results = [im find:query skip:1000 limit:1000 fields:nil sort:nil];
        ...
        describe(@"when using $gt operator", ^{
          ...
          it(@"works used with other predicates", ^{
            NSDictionary* query = @{ @"name" : @{@"$eq" : @"mike"}, @"age" : @{@"$gt" : @12} };
            CDTQResultSet* result = [im find:query];
            expect(result.documentIds.count).to.equal(2);
          });
        ...
        describe(@"when using $gte operator", ^{
          ...
          it(@"works used with other predicates", ^{
            NSDictionary* query = @{ @"name" : @{@"$eq" : @"mike"}, @"age" : @{@"$gte" : @12} };
            CDTQResultSet* result = [im find:query];
            expect(result.documentIds.count).to.equal(3);
          });
        });
        ...
        describe(@"when using $lt operator", ^{
          ...
          it(@"works used with other predicates", ^{
            NSDictionary* query = @{ @"name" : @{@"$eq" : @"mike"}, @"age" : @{@"$lt" : @12} };
            CDTQResultSet* result = [im find:query];
            expect(result.documentIds.count).to.equal(0);
          });
        ...
        describe(@"when using $lte operator", ^{
          ...
          it(@"works used with other predicates", ^{
            NSDictionary* query = @{ @"name" : @{@"$eq" : @"mike"}, @"age" : @{@"$lte" : @12} };
            CDTQResultSet* result = [im find:query];
            expect(result.documentIds.count).to.equal(1);
          });
        });
      ...
      describe(@"Executing queries using the $mod operator", ^{
        ...
        beforeEach(^{
          ...
          expect([im ensureIndexed:@[ @"name", @"score" ] withName:@"name_score"]).toNot.beNil();
        });
      ...
      describe(@"when using dotted notation", ^{
        ...
        it(@"query with two level dotted no results", ^{
          NSDictionary* query = @{ @"pet.name" : @{@"$eq" : @"fred"}, @"age" : @{@"$eq" : @12} };
          CDTQResultSet* result = [im find:query];
          expect(result).toNot.beNil();
        ...
        it(@"query with two level dotted one result", ^{
          NSDictionary* query = @{ @"pet.name" : @{@"$eq" : @"mike"}, @"age" : @{@"$eq" : @12} };
          CDTQResultSet* result = [im find:query];
      ...
      describe(@"when using $not", ^{
        ...
        beforeEach(^{
          ...
          expect([im ensureIndexed:@[ @"name", @"pet", @"age" ] withName:@"pet"]).toNot.beNil();
        });
        ...
        describe(@"when using $mod operator", ^{
          it(@"works used alone", ^{
            ...
            CDTQResultSet* result = [im find:query];
            expect(result.documentIds).to.containsInAnyOrder(@[ @"fred34", @"mike34", @"mike72" ]);
          });
          ...
          it(@"returns all docs when $mod applied to non-number field", ^{
            ...
            CDTQResultSet* result = [im find:query];
            expect(result.documentIds)
                .to.containsInAnyOrder(@[ @"fred12", @"mike12", @"fred34", @"mike34", @"mike72" ]);
          });
        });
      });
      ...
      describe(@"when indexing array fields", ^{
        ...
        beforeEach(^{
          ...
          rev = [CDTDocumentRevision revisionWithDocId:@"mike34"];
          rev.body = @{ @"name" : @"mike", @"age" : @34, @"pet" : @[ @"cat", @"dog", @"fish" ] };
          [ds createDocumentFromRevision:rev error:nil];
        ...
        it(@"finds documents with and without array", ^{
          ...
          expect(result).toNot.beNil();
          expect(result.documentIds.count).to.equal(3);
          expect(result.documentIds).to.containsInAnyOrder(@[ @"mike12", @"mike34", @"john22" ]);
        });
        ...
        it(@"works with $not", ^{
          ...
          expect(result.documentIds.count).to.equal(4);
          expect(result.documentIds)
              .to.containsInAnyOrder(@[ @"fred34", @"john44", @"john22", @"fred12" ]);
        });
        ...
        it(@"works with $gt", ^{
          ...
          expect(result).toNot.beNil();
          expect(result.documentIds.count).to.equal(3);
          expect(result.documentIds).to.containsInAnyOrder(@[ @"fred34", @"john44", @"mike34" ]);
        });
        ...
        it(@"works with $lte", ^{
          ...
          expect(result).toNot.beNil();
          expect(result.documentIds.count).to.equal(3);
          expect(result.documentIds).to.containsInAnyOrder(@[ @"mike12", @"john22", @"mike34" ]);
        });
        ...
        it(@"works with $not..$gt", ^{
          ...
          expect(result).toNot.beNil();
          expect(result.documentIds.count).to.equal(3);
          expect(result.documentIds).to.containsInAnyOrder(@[ @"fred12", @"john22", @"mike12" ]);
        });
        ...
        it(@"can find documents with and without arrays using $not multiple times", ^{
          ...
          expect(result).toNot.beNil();
          expect(result.documentIds.count).to.equal(3);
          expect(result.documentIds).to.containsInAnyOrder(@[ @"fred34", @"fred12", @"john44" ]);
        });
      ...
      describe(@"when using the $in operator", ^{
        ...
        it(@"can find documents using $not $in", ^{
          ...
          expect(result).toNot.beNil();
          expect(result.documentIds.count).to.equal(3);
          expect(result.documentIds).to.containsInAnyOrder(@[ @"fred12", @"fred34", @"john44" ]);
        });
      });
      ...
      describe(@"stopping enumeration", ^{
        ...
        describe(@"large result set", ^{
          ...
          it(@"limits correctly", ^{
            ...
            for (int i = 0; i < 100; i++) {
                rev = [CDTDocumentRevision revisionWithDocId:[NSString stringWithFormat:@"d%d", i]];
...
sharedExamplesFor(@"queries without covering indexes", ^(NSDictionary* data) {
  ...
  describe(@"when executing queries", ^{
    ...
    it(@"query using OR while missing a covering index", ^{
      NSDictionary* query =
          @{ @"$or" : @[ @{@"pet" : @{@"$eq" : @"cat"}}, @{@"town" : @{@"$eq" : @"bristol"}} ] };
      CDTQResultSet* result = [im find:query];
      expect(result).toNot.beNil();
      expect(result.documentIds.count).to.equal(4);
      expect(result.documentIds).to.beSupersetOf(@[ @"mike12", @"mike72", @"fred34", @"fred12" ]);
    });
  ...
  describe(@"when executing queries containing $size operator", ^{
    ...
    it(@"correctly performs compound AND query with single $size clause", ^{
      NSDictionary* query = @{ @"$and" : @[ @{@"pet" : @{@"$size" : @2}}, @{@"name" : @"mike"} ] };
      CDTQResultSet* result = [im find:query];
    ...
    it(@"correctly performs compound AND query on same field with single $size clause", ^{
      NSDictionary* query = @{ @"$and" : @[ @{@"pet" : @{@"$size" : @2}}, @{@"pet" : @"dog"} ] };
      CDTQResultSet* result = [im find:query];
    ...
    it(@"correctly performs compound OR query with single $size clause", ^{
      NSDictionary* query = @{ @"$or" : @[ @{@"pet" : @{@"$size" : @1}}, @{@"name" : @"john"} ] };
      CDTQResultSet* result = [im find:query];
      expect(result.documentIds)
          .to.containsInAnyOrder(@[ @"mike24", @"fred72", @"john12", @"john44" ]);
    });

    it(@"correctly performs compound OR query with multiple $size clauses on same field", ^{
      NSDictionary* query =
          @{ @"$or" : @[ @{@"pet" : @{@"$size" : @1}}, @{@"pet" : @{@"$size" : @2}} ] };
      CDTQResultSet* result = [im find:query];
      expect(result.documentIds)
          .to.containsInAnyOrder(@[ @"mike24", @"mike12", @"fred34", @"fred72", @"bill34" ]);
    });

    it(@"returns empty result set for compound AND, multiple $size clauses, same field", ^{
      NSDictionary* query =
          @{ @"$and" : @[ @{@"pet" : @{@"$size" : @1}}, @{@"pet" : @{@"$size" : @2}} ] };
      CDTQResultSet* result = [im find:query];
      expect(result.documentIds.count).to.equal(0);
    });
