...
- (void)setUp
{
    ...
    // class.
    self.expectedOutputStringFirstPart =
        @"<part the first, let us make it a bit longer for greater interest>";
    self.expectedOutputStringSecondPart = @"<2nd part, again unnecessarily prolonged for testing "
                                          @"purposes beyond any reasonable length...>";
    self.expectedOutputString = [self.expectedOutputStringFirstPart
        stringByAppendingString:self.expectedOutputStringSecondPart];
}
...
- (void)testCreateWriter
{
    ...
    XCTAssertEqual(stream.length, (SInt64)self.expectedOutputString.length,
                   @"unexpected string length in %s", __PRETTY_FUNCTION__);
}
...
- (void)testSynchronousWriter
{
    for (unsigned bufSize = 1; bufSize < 128; ++bufSize) {
        ...
        XCTAssertEqualObjects(outputBytes.my_UTF8ToString, self.expectedOutputString,
                              @"unexpected string in %s", __PRETTY_FUNCTION__);
        // Run it a second time to make sure re-opening works:
        outputBytes = [mp allOutput];
        XCTAssertEqualObjects(outputBytes.my_UTF8ToString, self.expectedOutputString,
                              @"unexpected string (2) in %s", __PRETTY_FUNCTION__);
    }
}
...
- (void)testASynchronousWriter
{
    ...
    [input close];
    [writer close];
    XCTAssertEqualObjects(tester->_output.my_UTF8ToString, self.expectedOutputString,
                          @"unexpected string in %s", __PRETTY_FUNCTION__);
}
