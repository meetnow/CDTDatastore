...
- (void)runReachability:(NSString *)hostname
{
    ...
    XCTAssertEqualObjects(r.hostName, hostname,
                          @"TDReachbility instance hostname (%@) is not %@ in %s:%@", r.hostName,
                          hostname, __PRETTY_FUNCTION__, hostname);
    ...
    r.onChange = ^{
      ...
      if (strongR.reachabilityKnown) resolved = YES;
    };
    XCTAssertTrue([r start], @"TDReachability failed to start in %s:%@", __PRETTY_FUNCTION__,
                  hostname);

    BOOL known = r.reachabilityKnown;
    //    NSLog(@"Initially: known=%d, flags=%x --> reachable=%d", known, r.reachabilityFlags,
    //    r.reachable);
    if (!known) {
        while (!resolved) {
            //            NSLog(@"waiting...");
