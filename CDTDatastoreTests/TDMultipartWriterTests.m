...
- (void)testSimpleMultiPartReadWithDelegate
{
    ...
    for (unsigned bufSize = 1; bufSize < expectedOutput.length + 1; ++bufSize) {
        TDMultipartWriter* mp =
            [[TDMultipartWriter alloc] initWithContentType:@"foo/bar" boundary:@"BOUNDARY"];

        XCTAssertEqualObjects(mp.contentType, @"foo/bar; boundary=\"BOUNDARY\"",
                              @"ContentType not equal in %s", __PRETTY_FUNCTION__);
        XCTAssertEqualObjects(mp.boundary, @"BOUNDARY", @"Boundary not equal in %s",
                              __PRETTY_FUNCTION__);
        ...
        XCTAssertEqual((NSUInteger)mp.length, expectedOutput.length,
                       @"Unexpected Writer output length in %s", __PRETTY_FUNCTION__);

        XCTAssertEqualObjects([[mp allOutput] my_UTF8ToString], expectedOutput,
                              @"Unexpected Writer output content in %s", __PRETTY_FUNCTION__);
        [mp close];
    }
}
