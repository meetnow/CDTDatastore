    ...
    describe(@"cloudant query", ^{
      ...
      describe(@"when sorting", ^{
        ...
        beforeEach(^{
          ...
          rev.body = @{ @"name" : @"fred", @"age" : @34, @"pet" : @"parrot", @"same" : @"all" };
          [ds createDocumentFromRevision:rev error:nil];
      ...
      describe(@"when generating ordering SQL", ^{
        ...
        __block NSDictionary *indexes = @{
            @"a" : @{@"name" : @"a", @"type" : @"json", @"fields" : @[ @"name", @"age", @"pet" ]},
        ...
        context(@"501 doc IDs", ^{
          ...
          context(@"for single field", ^{
            ...
            it(@"asc", ^{
              NSArray *order = @[ @{ @"name" : @"asc" } ];
              CDTQSqlParts *parts =
                  [CDTQQueryExecutor sqlToSortIds:largeDocIdSet usingOrder:order indexes:indexes];
        ...
        it(@"returns nil when no order", ^{
          CDTQSqlParts *parts;
          parts = [CDTQQueryExecutor sqlToSortIds:smallDocIdSet usingOrder:@[] indexes:indexes];
          expect(parts).to.beNil();
          parts = [CDTQQueryExecutor sqlToSortIds:smallDocIdSet usingOrder:nil indexes:indexes];
          expect(parts).to.beNil();
        });
