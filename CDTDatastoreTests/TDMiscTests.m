...
{
    XCTAssertEqualObjects(TDQuoteString(str1), str2,
                          @"TDQuoteString test failed for str1:%@ and str2:%@ in %s", str1, str2,
                          __PRETTY_FUNCTION__);
}

- (void)unquoteStringTest:(NSString *)str1 str2:(NSString *)str2
{
    XCTAssertEqualObjects(TDUnquoteString(str1), str2,
                          @"TDUnquoteString test failed for str1:%@ and str2:%@ in %s", str1, str2,
                          __PRETTY_FUNCTION__);
}

- (void)escapeIDTest:(NSString *)str1 str2:(NSString *)str2
{
    XCTAssertEqualObjects(TDEscapeID(str1), str2,
                          @"TDEscapeID test failed for str1:%@ and str2:%@ in %s", str1, str2,
                          __PRETTY_FUNCTION__);
}
...
- (void)testCleanURL
{
    ...
    testUrl = [NSURL
        URLWithString:@"https://adam:adamspassword@myhost.com:1234/db/with/long/path.html?q=true"];
    cleanString = TDCleanURLtoString(testUrl);
    XCTAssertEqualObjects(cleanString,
                          @"https://adam:*****@myhost.com:1234/db/with/long/path.html?q=true",
                          @"not cleaned: %@", cleanString);

    testUrl = [NSURL URLWithString:@"https://adam:adamspassword@myhost.com:1234/db/with/long/"
                                   @"path.html?q=true&foo=bar&bam=baz"];
    cleanString = TDCleanURLtoString(testUrl);
    ...
    cleanString = TDCleanURLtoString(testUrl);
    XCTAssertEqualObjects(cleanString, @"https://adam:*****@myhost.com/db", @"not cleaned: %@",
                          cleanString);

    testUrl = [NSURL
        URLWithString:@"https://adam:adamspassword@myhost.com:1234/db?all_docs=true#some_fragment"];
    cleanString = TDCleanURLtoString(testUrl);
    XCTAssertEqualObjects(cleanString,
    ...
    XCTAssertEqualObjects(cleanString, @"https://adam@myhost.com/db",
                          @"should not have changed: %@", cleanString);

    testUrl = [NSURL URLWithString:@"https://nothing.to.clean/db"];
    cleanString = TDCleanURLtoString(testUrl);
    XCTAssertEqualObjects(cleanString, @"https://nothing.to.clean/db",
                          @"should not have changed: %@", cleanString);
}
