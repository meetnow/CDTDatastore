...
- (const char *)encode:(id)obj
{
    ...
    NSString *str =
        [TDJSON stringWithJSONObject:obj options:TDJSONWritingAllowFragments error:NULL];
...
- (void)escapeTest:(const char *)source decodeChar:(char)decoded
{
    ...
    XCTAssertEqual(convertEscape(&pos), decoded, @"Decoder characters aren't equal (%c, %c) in %s",
                   convertEscape(&pos), decoded, __PRETTY_FUNCTION__);
...
- (int)collateLimited:(void *)mode
                 str1:(const void *)str1
                 str2:(const void *)str2
           arrayLimit:(unsigned)arrayLimit
{
...
- (void)scalarTest:(void *)mode
              str1:(const char *)str1
              str2:(const char *)str2
            retVal:(int)val
        arrayLimit:(unsigned)arrayLimit
{
    XCTAssertEqual([self collateLimited:mode
                                   str1:(const void *)str1
                                   str2:(const void *)str2
                             arrayLimit:arrayLimit],
                   val, @"with mode:%d and arrayLimit:%d, %s and %s do not collate to %d in %s",
                   *(unsigned *)mode, arrayLimit, str1, str2, val, __PRETTY_FUNCTION__);

    //(void*)mode used in these tests are defined in TDCollateJSON.h and may be cast as unsigned
    // integers.
}
